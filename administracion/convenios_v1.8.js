...
function guardaModificaConvenio(parametro) {
    $("#frmNuevoConvenio").validate({
        ...
        submitHandler: function() {
			...
			//var cobertura = $("#cmbTipoCobertura").val();
			var fecha_ini_fac = $("#txtRngFechaIni").val();
			var fecha_fin_fac = $("#txtRngFechaFin").val();
			var contratacion = $("#cmbTipoContratacion").val();
            ...
            if (filas >= "3") {
				var params = "opcion=3&parametro=" + parametro + "&nombre=" + nombre +
							 ...
							 "&fecha_ini_fac=" + fecha_ini_fac + "&fecha_fin_fac=" + fecha_fin_fac +
							 "&contratacion=" + contratacion + "&num_contrato=" + num_contrato;
...
function verificaGuardar() {
    ...
    if (resultado == "1") {
		...
		$("#contenedor_error").css("display", "block");
        $("#contenedor_error").html("Se ha producido un error al guardar los datos del convenio.");
	}
}
...
function verificaGuardaPlan() {
    var resultado = $("#hdd_resultado_plan").val();
  
    if (resultado > 0) {//Cierra la ventana flotante y Actualiza el listado de planes
        $("#contenedor_exito").css("display", "block");
