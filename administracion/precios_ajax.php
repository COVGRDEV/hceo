...
switch ($opcion) {
    ...
    case "3"://Imprime el listado de precios
        ...
        <div>
            ...
            <form id="frmListadoPrecios" name="frmListadoPrecios">
                <table style="width: 100%;">
                    ...
                    <tr>
                        <td>
                            <input type="text" id="txtParametroPrecios" name="txtParametroPrecios" placeholder="C&oacute;digo o nombre del procedimiento/medicamento/insumo" onblur="trim_cadena(this);" value="<?php echo($txtBuscar); ?>" />
                        </td>
                        <td style="width: 8%;">
                        ...
                        <td style="width: 20%;">
                            ...
                            <input type="button" id="btn_buscar_usuario" nombre="btn_buscar_usuario" value="Nuevo precio" class="btnPrincipal peq" onclick="ventanaAgregarPrecio(1, 0)"/>
                        </td>
                    </tr>
                </table>
            </form>
        ...
        <table class="paginated modal_table" style="width: 99%; margin: auto;">
            <thead>
                <tr>
                	<th colspan="10">Listado de precios</th>
                </tr>
                <tr>
                    <th style="width:10%;">Codigo</th>
                    <th style="width:10%;">Cod siesa</th>
			...
			?>
            <tr>
            	<td onclick="ventanaAgregarPrecio(1,<?php echo $value["id_precio"]; ?>)" align="center">
					<?php
						switch ($parametro) {
							case "P":
                ...
                <td onclick="ventanaAgregarPrecio(1,<?php echo $value["id_precio"]; ?>)" align="center"><?php echo($value["cod_insumo_siesa"]); ?></td>
                <td onclick="ventanaAgregarPrecio(1,<?php echo $value["id_precio"]; ?>)" align="left"><?php echo($nombre_prod_aux); ?></td>
                <td onclick="ventanaAgregarPrecio(1,<?php echo $value["id_precio"]; ?>)" align="center"><?php echo($value["fecha_ini_aux"]); ?></td>
                <td onclick="ventanaAgregarPrecio(1,<?php echo $value["id_precio"]; ?>)" align="center"><?php echo($value["fecha_final_aux"]); ?></td>
                <td onclick="ventanaAgregarPrecio(1,<?php echo $value["id_precio"]; ?>)" align="center">
					<?php
						...
						$codigo_servicio_siesa = $value["cod_insumo_siesa"];
					?>
                </td>
                <td onclick="ventanaAgregarPrecio(1,<?php echo $value["id_precio"]; ?>)" align="right"><?php echo("$".number_format($value["valor"])); ?></td>
                <td onclick="ventanaAgregarPrecio(1,<?php echo $value["id_precio"]; ?>)" align="right"><?php echo("$".number_format($value["valor_cuota"])); ?></td>
                <td onclick="ver_historia_precios(1,'<?php echo($codigo_servicio_siesa); ?>', <?php echo($tipo_bilateral); ?> )" align="right"><img src="../imagenes/ver_lista.png" title="Ver Precio"></td>
                <td align="right">
                	<?php
                    	if ($value["ind_activo_aux"] == "1") {
					?>
                	<img src="../imagenes/add_elemento.png" title="Agregar Precio" onclick="ventanaAgregarPrecio(2,<?php echo($value["id_precio"]); ?>)">
                    <?php
						}
					?>
                </td>
			...
			if ($tipo_accion == 1) {//Modificar precio
				if (count($rta_aux) >= 1) {
	                ...
	                if ($tipoServicio == "P") {
						...
						$cod_insumo_siesa = $rta_aux["cod_insumo_siesa"];
            ...
            <table style="width: 75%; margin: auto;">
                <tr>
                    ...
                    <td>
                        ...
                        <input type="hidden" id="hdd_idServicioSiesa" name="hdd_idServicioSiesa" value="<?php echo $cod_insumo_siesa ?>" />
                    </td>
                    <?php
    ...
    case "6"://Imprime el resultado de la busqueda para Servicios
        ...
        if ($tipoServicio == "P") {
            if ($indEcopetrol == "0") {
                $rta_aux = $dbMaestroProcedimientos->getProcedimientos($parametro, 1);
            } else if ($indEcopetrol == "1") {
                $rta_aux = $dbMaestroProcedimientos->getProcedimientosEcopetrol($parametro, 1);
            }
            ?>
            ...
            <table class="paginated modal_table" id="tablaServicios" style="width: 99%; margin: auto;">
                ...
                if (count($rta_aux) >= 1) {
                    foreach ($rta_aux as $value) {
                        ?>
                        <tr onclick="seleccionar_servicio('<?= $value["cod_insumo"]; ?>', '<?= $value["nombre_insumo"]; ?>', '<?= $value["cod_insumo_siesa"];?>')">
                            <td><?= $value["cod_insumo"]; ?></td>
                            <td style="text-align: left;"><?=  $value["nombre_insumo"]; ?></td>
    ...
    case "7"://Funcion que agrega el precio
		...
		$codigoServicioSiesa = $utilidades->str_decode($_POST["codigoServicioSiesa"]);
        ...
        $rta_aux = $dbListasPrecios->guardarEditarListasPrecios($accion, $tipoServicio, $fechaInicial, $fechaFinal,
				   $codServicio,  $plan, $tipoBilateral, $txtValorTotal, $txtValorCuota, $idUsuario, $idPrecio, $codigoServicioSiesa);
	?>
	...
	case "9"://Funcion que agrega el precio
        ...
        $rta_aux = $dbListasPrecios->GuardarListasPrecios($tipoServicio, $fechaInicial, $fechaFinal, $fechaFinalAnterior,
                ...
                <ol>
                	<?php
                   		if($tipo_servicio === "I"){
							...
							$tipo_liquidador = "(%)";
							$cod_item_siesa = " (ejemplo: LI-0001590 )";
                    ...
                    <li><strong>tipo liquidador </strong><?= $tipo_liquidador?></li>
                    <li><strong>c&oacute;digo siesa: </strong><?= $cod_item_siesa;?></li>
                </ol>
               </div>
    ...
    case "14": //Procesar archivo de precios CSV
        ...
        if ($fp = file($rutaFile)) {
            ...
            if ($filas > 0) {/* Valida Filas */
                ...
                /* Valida la versión del archivo - Deben de existir 8 columnas */
                $columnas = 0;
                    ...
                    for ($index = 1; $index <= $filas; $index++) {
                        ...
                        if ($contador < 1000) {
							...
							if ($cod_servicio != "") {
                            	...
                            	$arrDatos .= $cod_servicio.";".$valor.";".$fecha_ini.";".$fecha_fin.";".$valor_cuota.";".$tipo_bilateral.";".$liquidador.";".$tipo_liquidador.";".$cod_item_siesa;
							}
                        } else {
							if ($arrDatos != "") {
								$arrDatos .= "|";
	   ...
	   if($tipo_servicio === "P"){
		 ...
		 if(count($lista_precios)>=0){
			...
			$objPHPExcel->getActiveSheet()						 
						...
						->setCellValue("I".$contador_linea, "COD_ITEM_SIESA");
			...
			foreach($lista_precios as $value){
				...
				$objPHPExcel->getActiveSheet()
							->setCellValueExplicit("A".$contador_linea,$value["cod_servicio"],PHPExcel_Cell_DataType::TYPE_STRING)
							...
							->setCellValue("I".$contador_linea, $value["cod_item_siesa"]);
				$contador_linea++;
