...
function AgregarPrecio() {
    $("#frmAgregarPrecio").validate({
        ...
        submitHandler: function() {
			...
			var codigoServicioSiesa = $("#hdd_idServicioSiesa").val();
...
function verificaGuardarPrecio() {
    ...
    if (resultado == "-2") {
        ...
        $("#contenedor_error").html("Error al guardar. Revise que la fecha del registro no este en conflicto con un registro ya creado");
		 ...
		 $("#contenedor_error").css("display", "block");
        $("#contenedor_error").html("Error al interno al guardar el precio.");
	}else {
...
function seleccionar_servicio(idServicio, nombreServicio, codInsumoSiesa) {
    var codigoServicio = idServicio;

    //Asigna los valores 
    $("#txtNombreServicio").html(nombreServicio);
    $("#hdd_idServicio").val(codigoServicio);
	$("#hdd_idServicioSiesa").val(codInsumoSiesa);
...
function validarFrmPreciosCSV() {
    $("#frm_preciosCSV").validate({
        ...
        submitHandler: function () {
            ...
            llamarAjaxUploadFiles("precios_ajax.php", params, "resultadoImportarCSV", "validaSubirArchivo()", "", "fileCSV");
        },
    });
}
...
function validaSubirArchivo() {
	...
	var check_porc = $('#chk_porcentaje_xsx').prop('checked');
