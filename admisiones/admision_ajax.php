...
switch ($opcion) {
    ...
    case "3": //Crea la admision
		...
		@$cmb_profesionalFormula = $utilidades->str_decode($_POST["cmb_profesionalFormula"]);
		...
		@$cmb_zonificacion = $utilidades->str_decode($_POST["cmb_zonificacion"]);
		...
		@$txt_otro_merc = $utilidades->str_decode($_POST["txt_otro_merc"]);
		...
		$rta_aux = $dbAdmision->CrearEditarAdmision($in_nombre_1, $in_nombre_2, $in_apellido_1, $in_apellido_2, $in_id_tipo_documento, $in_numero_documento,
				...
				$cmbprofesionalAtiende, $cmb_profesionalFormula, $nombre_profesional, $arr_usuarios_citas_det, $arr_examenes, $in_id_usuario_crea, $hdd_id_admision_paciente, $hdd_estado_consulta,
				$in_id_estado_civil, $txt_num_carnet, $tipo_coti_paciente, $rango_paciente,$foto_paciente, $num_mipress,$num_ent_mipress, $num_poliza, $cmb_zonificacion);
		
		$rta_aux_merc = "";
		if($rta_aux>0){
			
			//INSERTAR LOS DATOS DE MERCADEO EN LA TABLA FORMULARIO MERCADEO
			$rta_aux_merc = $dbAdmision->CrearEditarDatosMercadeo($cmb_lista_merc, $cmb_subcategoria_merc, $txt_remitido_merc, $txt_otro_merc,$txt_referido_merc, $rta_aux);
                ...
                <tbody>
                    ...
                    if (count($pacientes2_aux) >= 1) {
                        foreach ($pacientes2_aux as $value) {
							...
							$telefono_1 = (count($ultima_admision) > 0 ? $value["telefono_1"] : "");
							$telefono_2 = (count($ultima_admision) > 0 ? $value["telefono_2"] : "");
							$email = $value["email"];
							$id_pais = $value["id_pais"];
							$cod_dep = $value["cod_dep"];
							$cod_mun = (count($ultima_admision) > 0 ? $value["cod_mun"] : "");
							$nom_dep = $value["nom_dep"];
							$nom_mun = (count($ultima_admision) > 0 ? $value["nom_mun"] : "");
							$direccion = (count($ultima_admision) > 0 ? $value["direccion"] : "");
							$id_zona = (count($ultima_admision) > 0 ? $value["id_zona"] : "");
							...
							$estado_civil = $value["id_estado_civil"];
							...
							if (count($ultima_admision) > 0) {
								...
								$id_convenio = ($id_convenio == "") ? $ultima_admision["id_convenio"] : $id_convenio;
							...
							<input type="hidden" name="hdd_id_convenio_<?php echo($id_paciente); ?>" id="hdd_id_convenio_<?php echo($id_paciente); ?>" value="<?php echo($id_convenio); ?>" />
                             <input type="hidden" name="hdd_id_estado_civil_<?php echo($id_paciente); ?>" id="hdd_id_estado_civil_<?php echo($id_paciente); ?>" value="<?php echo($estado_civil); ?>" />
                             <input type="hidden" name="hdd_id_foto_paciente_<?php echo($id_paciente); ?>" id="hdd_id_foto_paciente_<?php echo($id_paciente); ?>" value="<?php echo($foto_paciente); ?>" />
    ...
    case "7": //Verifica si el paciente existe
        ...
        $resultado = $dbPacientes->getPacienteNumeroDocumento($ndocumento);
    ...
    case "8": //Dibuja el div de precios
		...
		if ($modificar_precios == 1) {
			$disabled_modificar_precios = "";
			$modificar_precios = true;
		} else if($modificar_precios == 0) {
			$disabled_modificar_precios = 'disabled="disabled"';
        	...
        	<table id="tablaPrecios" class="modal_table">
                <tr id="tabla_encabezado">
                    ...
                    <th style="width:6%;" id="icono">
                    	<?php
                    		if ($modificar_precios == true && $status_convenio != "2") {
                    	?>
							...
							if ($bol_mostrar) {
								...
								switch ($precio_aux["tipo_precio"]) {
									...
									case "I":
										...
										$cod_servicio_aux_siesa = $precio_aux["cod_insumo_siesa"];
										break;
                ...
                <tr id="tr_precios_<?php echo($p); ?>">
                	<td align="left" class="td_reducido">
                        ...
                        <input type="hidden" name="hdd_cod_servicio_<?php echo($p); ?>" id="hdd_cod_servicio_<?php echo($p); ?>" value="<?php echo($cod_servicio_aux); ?>" />
                         <input type="hidden" name="hdd_cod_servicio_siesa_<?php echo($p); ?>" id="hdd_cod_servicio_siesa_<?php echo($p); ?>" value="<?php echo($cod_servicio_aux_siesa); ?>" />
                    ...
                    <td align="left" class="td_reducido">
                        ...
                        <div id="d_convenio_pago_<?php echo($p); ?>" style="width:100%;">
                        	...
                        	<?php
								$combo->getComboDb("cmb_convenio_pago_".$p, $id_convenio_aux, $lista_convenios, "id_convenio, nombre_convenio", "--Seleccione el convenio--", "seleccionar_convenio_pago(".$p.", this.value, '');", true, "width:100%; margin:0 0 2px 0;");
							?>
                        </div>
                        <div id="d_plan_pago_<?php echo($p); ?>" style="width:100%;">
                            <?php
								$combo->getComboDb("cmb_plan_pago_".$p, $id_plan_aux, $lista_planes, "id_plan, nombre_plan", "--Seleccione el plan--", "actualizar_precio_det(".$p.", this.value);", true, "width:100%; margin:0 0 2px 0;");
							?>
                        </div>
                        <div id="d_plan_pago_sel_<?php echo($p); ?>" style="display:none;"></div>
                    </td>
                    <td align="center">
                    	<input type="text" name="	<?php echo($p); ?>" id="txt_num_autorizacion_<?php echo($p); ?>" value="<?php echo($precio_aux["num_autorizacion"]); ?>" class="no-margin" maxlength="20" onblur="trim_cadena(this);" <?php if (!$modificar_precios || $ind_num_aut_aux != "1") { ?>disabled="disabled"<?php } ?> />
                    </td>
                    <td align="center">
                    	<?php
                    ...
                    <td class="td_reducido">
                    	<input type="text" name="txt_cant_precio_<?php echo($p); ?>" id="txt_cant_precio_<?php echo($p); ?>" value="<?php echo($precio_aux["cantidad"]); ?>" class="texto_centrado no-margin" maxlength="3" onkeypress="return solo_numeros(event, false);" onchange="recalcular_totales_liquidacion();" <?php echo($disabled_modificar_precios); ?> />
                    </td>
                    <td class="td_reducido">
                    	<?php
							if ($bol_np) {
                        ...
                        <input type="text" name="txt_valor_pago_<?php echo($p); ?>" id="txt_valor_pago_<?php echo($p); ?>" value="<?php echo($valor_aux); ?>" class="texto_centrado no-margin" maxlength="9" onkeypress="return solo_numeros(event, false);" onchange="recalcular_totales_liquidacion();" <?php echo($disabled_modificar_precios); ?> />
                    </td>
                    <td class="td_reducido">
                    	<?php
	...
	case "9": //Dibuja el div Flotante de Todos los precios
        ...
        <form id="frmBuscarPrecios">
            <table>
                <tr>
                    <td>
                        <input type="text" id="txt_identificacion_interno" name="txt_identificacion_interno" placeholder="Digite el c&oacute;digo o nombre del producto o servicio" style="width:600px;" />
                    </td>
                    <td>
    ...
    case "10": //Dibuja el div Flotante de Todos los precios
		...
		$aux_paquetes = $dbPaquetesProcedimientos->buscarPaquetes($parametro, 1, $id_convenio);
		...
		if (count($aux_paquetes) > 0) {
			...
			?>
 				<input type="hidden" name="hdd_tipo_precio_paquete" id="hdd_tipo_precio_paquete" value="<?= $pecio_aux["tipo_precio"]; ?>" />            
            <?php
			}
		}
		...
		if (count($aux_precios_otros) > 0) {
			foreach ($aux_precios_otros as $otro_aux) {
				...
				?>
                <input type="hidden" name="hdd_tipo_precio_paquete" id="hdd_tipo_precio_paquete" value="0" />    
                <?php
		...
		if (count($aux_precios) > 0) {
						...
						foreach ($aux_precios as $value) {
							...
							switch ($value["tipo_precio"]) {
								...
								case "I":
									...
									$cod_servicio_siesa_aux = $value["cod_insumo_siesa"];
									break;
								case "Q":
                    ...
                    <tr id="<?php echo("b".$contador); ?>">
                        ...
                        <td align="left">
								...
								foreach ($lista_precios_paquete_det as $prod_aux) {
										...
										switch ($prod_aux["tipo_producto"]) {
											...
											case "I":
												...
												$cod_servicio_siesa_b_det=$prod_aux["cod_insumo_siesa"];
												break;
										}
                            ...
                            <input type="hidden" name="hdd_cod_servicio_b_det_<?= $contador."_".$cont_aux_p ?>" id="hdd_cod_servicio_b_det_<?= $contador."_".$cont_aux_p ?>" value="<?= $cod_servicio_b_det ?>" />
                                 <input type="hidden" name="hdd_cod_servicio_siesa_b_det_<?= $contador."_".$cont_aux_p ?>" id="hdd_cod_servicio_siesa_b_det_<?= $contador."_".$cont_aux_p ?>" value="<?= $$cod_servicio_siesa_b_det ?>" />
                        ...
                        <td align="center">
                            ...
                            <input type="hidden" id="hdd_cod_servicio_b_<?php echo($contador); ?>" value="<?php echo($cod_servicio_aux); ?>" />
                             <input type="hidden" id="hdd_cod_servicio_siesa_b_<?php echo($contador); ?>" value="<?php echo($cod_servicio_siesa_aux); ?>" />
			...
			?>
            <input id="btnModalAgregar" name="btnModalAgregar" class="btnPrincipal" type="button" value="Agregar" onclick="agregarPrecios();" />
            <?php
        } else {
            ?>
    ...
    case "11": //Guardar lista de precios en tabla temporal
		...
		for ($i = 0; $i < $cantPrecios; $i++) {
			...
			@$codServicioSiesa = $utilidades->str_decode($_POST["cod_servicio_siesa_".$i]);
			...
			switch ($tipoPrecio) {
				...
				case "I":
					...
					$cod_insumo_siesa= $codServicioSiesa;
					break;
			}
			...
			$resultado_aux = $dbAdmision->guardarTemporalPagosDetalle($idUsuario, $idListaPrecios, $idConvenioPago, $idPlanPago, $numAutorizacion, $tipoBilateral,
							 $cantidad, $tipoPrecio, $valor, $indTipoPago, $valorCuota, $cod_procedimiento, $cod_medicamento, $cod_insumo, $cod_insumo_siesa);
	...
	case "17": //Buscar valores de un plan seleccionado para precios
		...
		@$tipo_paquete = $utilidades->str_decode($_POST["tipo_paquete"]);
		
		//var_dump($tipo_paquete. " TIPO PAQUETE");
	...
	case "18": //Búsqueda de número de carnet en atenciones anteriores
		@$id_convenio = $utilidades->str_decode($_POST["id_convenio"]);
		@$id_paciente = $utilidades->str_decode($_POST["id_paciente"]);
		$num_carnet = "";
	...
	case "21":
		...
		$otro = $utilidades->str_decode($_POST["otro"]);
	...
	?>
        ...
        <form id="frmFlotanteMercadeo">
            <table id="tabla_refrafinal" class="modal_table" cellpadding="5" cellspacing="0" align="center" style="width:95%; margin-top:20px; margin-bottom:15px;">
              <tr>
             	<tr>
	...
	?>
     ...
     <input type="hidden" name="hdd_otro_<?=$id_paciente?>" id="hdd_otro_<?=$id_paciente?>" value="<?= $otro?>">
	...
	if($id_categoria == 89){
		...
		<td align="left" id="td_remitido_merc">
			<input type="text" name="txt_remitido_merc" id="txt_remitido_merc" value="<?= $remitido?>" placeholder="Ingrese quién lo remitió">
		</td>
        <?php
			
	}else if($id_categoria == 91){
		
		?>
		<td align="left" id="td_remitido_merc">
			<input type="text" name="txt_otro_merc" id="txt_otro_merc" value="<?= $otro?>" placeholder="Ingrese el medio">
		</td>
		<?php
		
	}else if($id_categoria == 97){
		?>
		<td align="left" id="td_remitido_merc">
		...
		<?php
			...
			echo $combo->getComboDb("cmb_subcategoria_merc", $id_detalle, $lista_categorias, "id_detalle, nombre_detalle", "----Seleccione----", "", "", "width: 188px;");
		}	
	break;
	...
	$dir = $dbVariables->getURLFotoPaciente();
	$dir = $dir["valor_variable"];
	
     if(!is_dir($dir)){
       @mkdir($dir);
     }
	$nombre_foto = "f_".$numero_documento.".jpeg";
	$file = $dir.$nombre_foto;	
	$success = file_put_contents($file, $data);
	$data = base64_decode($data); 
	$source_img = imagecreatefromstring($data);
	$dest = imagecreatetruecolor(80, 40);
	// Copy
	imagecopy($dest, $source_img, 0, 0, 20, 13, 80, 40);	

	$imageSave = imagejpeg($source_img, $file, 100);
	imagedestroy($source_img);
	?>
    	<input type="hidden" id="foto_paciente_saved" value="<?= $nombre_foto?>" />
    <?php	
