...
function mostrar_formulario_flotante_adm(tipo) {
    if (tipo == 1) {//mostrar
        ...
        $("#a_cierre_panel2").remove();//Elimina el  icono cerrar ventana agregado en las lineas de arriba
...
function mostrarFormularioMercadeo(){
	...
	if(categoria=="89" || categoria=="91" || categoria=="97"){
...
function seleccionar_mercadeo(){
	...
	var remitido = "";
	var otro = "";
	var referido = $("#hdd_referido_mercadeo_form").val();	
...
function seleccionUsuario(id) {//Evento clic en el formulario flotante: Buscar Usuario
    ...
    if (confirm("\u00BFConfirma que desea seleccionar el paciente " + nombre_completo_aux + "?")) {
		...
		$("#cmb_estado_civil").val($("#hdd_id_estado_civil_" + id).val());
		$("#hdd_foto_paciente").val($("#hdd_id_foto_paciente_" + id).val());
		...
		seleccionar_zona($("#hdd_id_lista_detalles_" + id).val(), 1);
...
function guardar_admision() {
	...
	$("#frmAdmision").validate({
        ...
        submitHandler: function(form) {
			...
			if ($("#cmb_usuario_prof_formula").is(":visible")) {
                cmb_profesionalFormula = $("#cmb_usuario_prof_formula").val().length > 0 ? $("#cmb_usuario_prof_formula").val() : "-1";//-1 el objeto esta visible
            } else {
                cmb_profesionalFormula = "NULL";//-2 El objeto no esta visible
            }
			
			if ($("#txt_nombre_prof_formula").is(":visible")) {
                nombre_profesional = $("#txt_nombre_prof_formula").val().trim().length > 0 ? $("#txt_nombre_prof_formula").val().trim() : "-1";//-1 el objeto esta visible
            } else {
                nombre_profesional = "NULL";//-2 El objeto no esta visible
            }
			...
			if ($("#td_num_mipress").is(":visible")) {
                num_mipress = $("#txt_num_mipress").val().length > 0 ? $("#txt_num_mipress").val() : "NULL";//-1 el objeto esta visible
            } else {
                num_mipress = "NULL";//-2 El objeto no esta visible
            }	
			
			if ($("#td_num_entrega_mipress").is(":visible")) {
                num_ent_mipress = $("#txt_num_entrega_mipress").val().length > 0 ? $("#txt_num_entrega_mipress").val() : "NULL";//-1 el objeto esta visible
            } else {
                num_ent_mipress = "NULL";//-2 El objeto no esta visible
            }
			
			if ($("#td_num_poliza").is(":visible")) {
                num_poliza = $("#txt_num_poliza").val().length > 0 ? $("#txt_num_poliza").val() : "NULL";//-1 el objeto esta visible
            } else {
                num_poliza = "NULL";//-2 El objeto no esta visible
            }
            ...
            if (cmb_departamento_nac === "-1" || cmb_municipio_nac === "-1" || txt_nombre_dep_nac === "-1" || txt_nombre_mun_nac === "-1"
					...
					|| cmb_desplazado === "-1" || cmb_etnia === "-1" || cmd_profesionalAtiende === "-1" ||  cmb_profesionalFormula === "-1" 
					||  nombre_profesional === "-1" || txt_nombre_cirugia === "-1"
                ...
                cmd_profesionalAtiende === "-1" ? $("#cmd_profesionalAtiende").addClass("bordeAdmision") : $("#cmd_profesionalAtiende").removeClass("bordeAdmision");
				 cmb_profesionalFormula === "-1" ? $("#cmb_usuario_prof_formula").addClass("bordeAdmision") : $("#cmb_usuario_prof_formula").removeClass("bordeAdmision");
				
				
				nombre_profesional === "-1" ? $("#txt_nombre_prof_formula").addClass("bordeAdmision") : $("#txt_nombre_prof_formula").removeClass("bordeAdmision");
				...
				num_mipress === "-1" ? $("#txt_num_mipress").addClass("bordeAdmision") : $("#txt_num_mipress").removeClass("bordeAdmision");
				num_ent_mipress === "-1" ? $("#txt_num_entrega_mipress").addClass("bordeAdmision") : $("#txt_num_entrega_mipress").removeClass("bordeAdmision");
				num_poliza === "-1" ? $("#txt_num_poliza").addClass("bordeAdmision") : $("#txt_num_poliza").removeClass("bordeAdmision");
				...
				$("#cmb_usuario_prof_formula").removeClass("bordeAdmision");
				$("#txt_nombre_prof_formula").removeClass("bordeAdmision");
				...
				$("#txt_num_mipress").removeClass("bordeAdmision");
				$("#txt_num_entrega_mipress").removeClass("bordeAdmision");
				$("#txt_num_poliza").removeClass("bordeAdmision");
				$("#txt_email").removeClass("bordeAdmision");
...
function continuar_guardar_admision() {
	...
	var cont_aux = 0;
	var filas = parseInt($("#hdd_ult_index_precio").val(), 10);
	var params = "opcion=11&id_medio_pago=" + $("#cmb_medio_pago").val();
	for (i = 1; i <= filas; i++) {
		...
		if (isObject(document.getElementById("tr_precios_" + i))) {
			params += "&idListaPrecios_" + cont_aux + "=" + $("#hdd_id_precio_" + i).val() +
					  ...
					  "&cod_servicio_siesa_" + cont_aux + "=" + $("#hdd_cod_servicio_siesa_" + i).val() +
...
function continuar_guardar_admision_2() {
	...
	if($("#hdd_categoria_mercadeo").val() != '' && ($("#hdd_subcategoria_mercadeo").val() != '' ||
		$("#hdd_remitido_mercadeo").val() != '' || $("#hdd_otro_mercadeo").val() ||  $("#hdd_referido_mercadeo").val())){
	...
	if (tmp_pagos_detalle <= 0 || ind_continuar_mercadeo == false) {
		...
		var cmb_zonificacion = $("#cmb_zonificacion").val();
		var foto_paciente = $("#hdd_foto_paciente").val() != '' ? $("#hdd_foto_paciente").val() : "NULL";
		...
		var params = "opcion=3&txtnombre1=" + str_encode($("#txt_nombre").val()) +
					 ...
					 "&cmb_profesionalFormula=" + cmb_profesionalFormula +
...
function verificar_guardar_admision() {
	...
	var resultado_aux_merc = ($("#hdd_resultado_crear_form_mercadeo").val() != "" ? parseInt($("#hdd_resultado_crear_form_mercadeo").val(), 10) : -200);
...
function opcionesFormularioflotante(n1) {
    if (n1 == 1) {
        ...
        if (datosPacienteCedula[0] != "0") {
			...
			$("#hdd_categoria_mercadeo").val(resultado[40]);
			$("#hdd_subcategoria_mercadeo").val(resultado[41]);
			$("#hdd_remitido_mercadeo").val(resultado[42]);
			$("#hdd_referido_mercadeo").val(resultado[43]);
			$("#hdd_otro_mercadeo").val(resultado[44]);
			...
			$("#cmb_numerohermanas").val(trim(resultado[38]));
			$("#cmb_estado_civil").val(trim(resultado[30]));
            $("#cmb_id_pais_nac").val(resultado[31]);
			seleccionar_pais(resultado[31], "_nac");
            $("#cmb_departamento_nac").val(resultado[32]);
		...
		//Se carga de nuevo el componente de historia clínica
		cargar_componente_hc_adm(resultado[36]);
...
function seleccionar_convenio(id_plan, modificar_valores) {
	...
	if (ind_eco == "1") {
		...
		$("#td_zonificacion_l").css("display", "table-cell");
		$("#td_zonificacion").css("display", "table-cell");
	} else {
		$("#tr_nombre_med_orden_label").css("display", "none");
		$("#tr_nombre_med_orden").css("display", "none");
		$("#txt_nombre_med_orden").val("");
		$("#td_zonificacion_l").css("display", "none");
		$("#td_zonificacion").css("display", "none");
		$("#cmb_zonificacion").val("");
	}
	...
	if (ind_num_carnet == "1") {
		...
		$("#td_num_entrega_mipress_l").css("display", "table-cell");
		$("#td_num_entrega_mipress").css("display", "table-cell");
		...
		$("#td_num_entrega_mipress_l").css("display", "none");
		$("#td_num_entrega_mipress").css("display", "none");
		$("#txt_num_entrega_mipress").val("");
...
function seleccionarProfesional(){
	...
	if($("#cmb_usuario_prof_formula").val() == "234"){
		$(".td_nombre_prof").css("display","table-cell")
	}else{
		$(".td_nombre_prof").css("display","none");
		$("#txt_nombre_prof_formula").val("");
	}
			...
			$("#tablaPrecios").append(
				'<tr id="tr_precios_' + cont_aux + '">\n' +
					'<td align="left" class="td_reducido">\n' +
						...
						'<input type="hidden" name="hdd_cod_servicio_siesa_' + cont_aux + '" id="hdd_cod_servicio_siesa_' + cont_aux + '" value="' + $("#hdd_cod_servicio_siesa_b_" + i).val() + '" />\n' +
					...
					'<td class="td_reducido">\n' +
						...
						'<input type="text" name="txt_valor_pago_' + cont_aux + '" id="txt_valor_pago_' + cont_aux + '" value="' + $("#hdd_precio_b_" + i).val() + '" class="texto_centrado no-margin" maxlength="9" onkeypress="return solo_numeros(event, false);" onchange="recalcular_totales_liquidacion();" />\n' +
					'</td>\n' +
					'<td>\n' +
						'<input type="text" name="txt_valor_cuota_' + cont_aux + '" id="txt_valor_cuota_' + cont_aux + '" value="' + $("#hdd_precio_cuota_b_" + i).val() + '" class="texto_centrado no-margin" maxlength="9" onkeypress="return solo_numeros(event, false);" onchange="recalcular_totales_liquidacion();" ' + disabled_aux + ' />\n' +
					'</td>\n' +
					'<td style="width: 40px;">\n' +
			...
			for (j = 0; j < cantidad_b_det; j++) {
				...
				$("#tablaPrecios").append(
					'<tr id="tr_precios_' + cont_aux + '">\n' +
						...
						'<td class="td_reducido">\n' +
							'<input type="text" name="txt_cant_precio_' + cont_aux + '" id="txt_cant_precio_' + cont_aux + '" value="1"  disabled="disabled" class="texto_centrado no-margin" maxlength="3" onkeypress="return solo_numeros(event, false);" onchange="recalcular_totales_liquidacion();"/>\n' +
						'</td>\n' +
						'<td class="td_reducido">\n' +
							'<input type="hidden" name="hdd_tipo_pago_' + cont_aux + '" id="hdd_tipo_pago_' + cont_aux + '" value="' + $("#hdd_ind_tipo_pago_b_det_" + i + "_" + j).val() + '" />\n' +
							'<input type="text" name="txt_valor_pago_' + cont_aux + '" id="txt_valor_pago_' + cont_aux + '" value="' + $("#hdd_valor_b_det_" + i + "_" + j).val() + '"  disabled="disabled" class="texto_centrado no-margin" maxlength="9" onkeypress="return solo_numeros(event, false);" onchange="recalcular_totales_liquidacion();" ' + disabled_aux + ' />\n' +
						'</td>\n' +
						'<td>\n' +
							'<input type="text" name="txt_valor_cuota_' + cont_aux + '" id="txt_valor_cuota_' + cont_aux + '" value="' + $("#hdd_valor_cuota_b_det_" + i + "_" + j).val() + '" disabled="disabled" class="texto_centrado no-margin" maxlength="9" onkeypress="return solo_numeros(event, false);" onchange="recalcular_totales_liquidacion();" ' + disabled_cuota_aux + '  />\n' +
						'</td>\n' +
						'<td style="width: 40px;">\n' +
							'<div class="Error-icon" onclick="borrarPrecio(\'tr_precios_' + cont_aux + '\');"></div>\n' +
						'</td>\n' +
...
function actualizar_precio_det(indice, id_plan) {
	...
	var tipo_paquete = $("#hdd_tipo_precio_paquete").val();
	tipo_paquete == "Q" ? tipo_paquete : 0; 
	
	//window.alert(tipo_paquete + " Tipo precio");
...
//Foto
//Capturar foto
var localstream, canvas, video, cxt, listaDeDispositivos;
...
function abrirCamara(){
	...
	if(numero_documento != "" && numero_documento != undefined){
		...
		swal({
			title: "Error al abrir la cámara",
			type: "error",
...
function terminar_guardar_foto(){
	...
	if($("#foto_paciente_saved").val() != "" || $("#foto_paciente_saved").val() != undefined){
