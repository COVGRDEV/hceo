<?php
	...
	require_once("../funciones/Class_Clientes_SIESA.php");
	require_once("../funciones/Class_Pedidos_SIESA_v2.php");
	require_once("../funciones/Class_Facturas_SIESA_v2.php");
	...
	$classClientesSIESA = new Class_Clientes_SIESA();
	$classPedidosSIESAv2 = new Class_Pedidos_SIESA_v2();
	$classFacturasSIESAv2 = new Class_Facturas_SIESA_v2();
	...
	switch ($opcion) {
        ...
        <?php
			...
			<div style="margin-bottom: 30px;" id="a<?php echo($contadorid_aux); ?>"> 
				<table style="width:100%; margin:auto;" border="0">
                    ...
                    <tr>
                        <td style="text-align:right;">
							<label class="inline no-margin">Profesional formula:&nbsp;</label>
						</td>
						<td style="text-align:left;">
							<?php
								$id_usuario_prof_formula = "0";
								
								
								if ($ind_crear == 0) {
									$id_usuario_prof_formula = $value["id_usuario_prof_formula"];
								}
								
								if ($ind_crear != 0 || /*$ind_modificar_pagos == 1 ||*/ $value["id_tipo_cita"] == "5" || $value["id_tipo_cita"] == "14" || $value["id_tipo_cita"] == "15" || $value["id_tipo_cita"] == "16" || $value["id_tipo_cita"] == "25" || $value["id_tipo_cita"] == "20") { 
								
									$lista_profesionales_aux = $dbUsuarios->getProfesionalesPorPerfil("3");
								
									$combo->getComboDb("cmb_usuario_prof_formula_".$contadorid_aux, $id_usuario_prof_formula, $lista_profesionales_aux, "id_usuario,profesional", "--Seleccione el profesional--", "seleccionarProfesional();", true, "width:200px;", "", "no-margin");
									
								} else {
									$usuario_obj = $dbUsuarios->getUsuario($id_usuario_prof_formula);
							?>
                            <input type="hidden" id="cmb_usuario_prof_formula_<?php echo($contadorid_aux); ?>" name="cmb_usuario_prof_formula_<?php echo($contadorid_aux); ?>" value="<?php echo($id_usuario_prof_formula); ?>" />
                            <label class="inline no-margin"><b><?php echo($usuario_obj["nombre_usuario"]." ".$usuario_obj["apellido_usuario"]); ?></b></label>
                         <?php } ?>
                           							
						</td>
                        <?php 
							$display_nombre_prof = "display:none";
							if($ind_crear == 0){
								$display_nombre_prof = "display:table-cell";
							}
						?>
						<td class="td_profesional_formula" style="text-align:right; <?= $display_nombre_prof ?>">
                         	<label class="inline no-margin">Nombre Profesional:&nbsp;</label>
                        </td>
                       	
						<td class="td_profesional_formula" style="text-align:left; <?= $display_nombre_prof ?>">
                        <?php 
						if ($ind_crear == "1" || /*$ind_modificar_pagos == 1 ||*/ $value["id_tipo_cita"] == "5" || $value["id_tipo_cita"] == "14" || $value["id_tipo_cita"] == "15" || $value["id_tipo_cita"] == "16" || $value["id_tipo_cita"] == "25" || $value["id_tipo_cita"] == "20") { 
						
						?>
                        	<input style="width:200px;" type="text" id="txt_profesional_formula" name="txt_profesional_formula" value="<?= $value["nombre_prof_formula"]  ?>" />
						<?php 
						} else{
							?>
                        		<input style="width:200px;" type="hidden" id="txt_profesional_formula" name="txt_profesional_formula" value="<?= $value["nombre_prof_formula"]; ?>" />
                            <label class="inline no-margin"><b><?= $value["nombre_prof_formula"] == "" ? $usuario_obj["nombre_usuario"]." ".$usuario_obj["apellido_usuario"] :  $value["nombre_prof_formula"] ?></b></label>
                    ...
                    <?php 
						...
						if(isset($value["id_paciente"])){
								
							$num_mipress = $value["num_mipress"];
							$num_ent_mipress = $value["num_ent_mipress"];
							$tipo_paciente = $value["tipo_coti_paciente"];
							$num_poliza = $value["num_poliza"];
						}
					?>
                      <input type="hidden" id="hdd_tipo_paciente" name="hdd_tipo_paciente"  value="<?= $tipo_paciente ?>" />
                      <input type="hidden" id="hdd_num_mipress" name="hdd_num_mipress"  value="<?= $num_mipress ?>" />
                      <input type="hidden" id="hdd_num_ent_mipress" name="hdd_num_ent_mipress"  value="<?= $num_ent_mipress ?>" />
                      <input type="hidden" id="hdd_num_poliza" name="hdd_num_poliza"  value="<?= $num_poliza ?>" />
					...
					if($ind_crear == 1 || $ind_modificar_pagos == 1){ 
					
					?>
                     <tr>
                    	<td style="text-align:right;">
							<label class="inline no-margin">Tipo paciente:</label>
						</td>
						<td style="text-align:left;">
							<?php 
								
                            	$combo->getComboDb("cmb_tipo_paciente", $tipo_paciente, $dbListas->getListaDetalles(101), "id_detalle, nombre_detalle", "Seleccione---", "", "", "width:200px; margin:0; display:block;");
                            ?>
						...
						<td id="td_num_poliza" style="text-align:left; display:<?= $display; ?>;">
							<input <?= $disabled ?>  placeholder="<?= $num_poliza ?>" style="width: 200px; margin:0;" type="text" id="txt_num_poliza" name="txt_num_poliza"  value="" maxlength="20" onblur="trim_cadena(this);" />
						</td>
                    </tr>
                    ...
                    <tr>
						...
						<td id="td_num_mipress" style="text-align:left; display:<?= $display; ?>;">
							 <input <?= $disabled ?> placeholder="<?= $num_mipress ?>" style="width: 200px; margin:0;" type="text" id="txt_num_mipress" name="txt_num_mipress"  value="" maxlength="20" onblur="trim_cadena(this);" />
						</td>
						...
						<td id="td_num_ent_mipress" style="text-align:left; display:<?= $display; ?>;">
							 <input <?= $disabled ?> placeholder="<?= $num_ent_mipress ?>" style="width: 200px; margin:0;" type="text" id="txt_num_entrega_mipress" name="txt_num_entrega_mipress"  value="" maxlength="20" onblur="trim_cadena(this);" />
						</td>
                    </tr>
                   <?php } ?>
				</table>
				...
				<table class="modal_table" id="tablaPrecios<?php echo($contadorid_aux); ?>" style="width:100%;">
					...
					<?php
						...
						foreach ($resultado2_aux as $value2) {
							...
							if ($value2["tipo_precio"] == "P") {
								...
								$cod_aux_siesa=$value2["cod_insumo_siesa"];
							}
					?>
					<tr id="tr_producto_<?php echo($cont_aux); ?>">
						<td align="center">
                            ...
                            <input type="hidden" name="hdd_cod_servicio_siesa_<?php echo($cont_aux); ?>" id="hdd_cod_servicio_siesa_<?php echo($cont_aux); ?>" value="<?= $cod_aux_siesa ?>" />
					...
					<?php
							...
							$total += $value2["total"];
							$valor_cuota_total += intval($value2["valor_cuota"]);
							$cont_aux++;
						}
					?>
				</table>
				...
				<div style="width:800px; margin:auto;">
					<table border="0" cellpadding="5" cellspacing="0" style="width: 100%;">
						...
						<tr>
							...
							<td align="right" style="width:22%;"><label>N&uacute;mero de factura<?php /*if ($ind_factura_obligatoria == 1) { ?>*<?php }*/ ?></label></td>
                            	...
                            	<?php
									...
									if ($ind_crear == 0) {
										if (isset($value["estado_pago"]) && $value["estado_pago"] == "1" && $tipo_acceso_menu == "2" && !$ind_pasado && $ind_modificar_pagos == 1) {
											//Mostrar input text
								?>
                                <input  type="text" id="txt_factura" name="txt_factura"  value="<?php echo($num_factura); ?>" placeholder="<?php /*$value["num_factura"]*/?>Factura" maxlength="20" onblur="trim_cadena(this);" /> 
								<?php
                                ...
                                <?php
									...
									if ($ind_crear == 0) {
										if (isset($value["estado_pago"]) && $value["estado_pago"] == "1" && $tipo_acceso_menu == "2" && !$ind_pasado && $ind_modificar_pagos == 1) {	
											//Mostrar input text
								?>
                                		<input  type="text" id="txt_pedido" name="txt_pedido"  value="<?php echo($num_pedido_aux); ?>" placeholder="Pedido" maxlength="20" onblur="trim_cadena(this);" onkeypress="solo_numeros(event, false);" />
								<?php	
										}else if (isset($value["estado_pago"]) && $value["estado_pago"] == "2" && $tipo_acceso_menu == "2"  && $ind_modificar_pagos == 1){
												?>
					...
					<div>
						...
						?>
                        <input class="btnPrincipal peq" type="button" name="btnConsultarSiesa" id="btnConsultarSiesa" value="Consultar pago en SIESA" onclick="consultar_pago_siesa('<?= $value["id_pago"]; ?>', '<?= $value["id_lugar_cita"]; ?>');" />
                        <div id="d_tipo_pago_siesa" class="rojo"></div>
                        <?php
							}
						?>																															 
                       </div>
                ...
                <?php
								...
								} else if ($ind_modificar_pagos == 1 && $ind_registrar_pagos == 1) {
				?>
		...
		case "2": //Registra el Pago
			...
			@$idUsuarioProfFormula = $utilidades->str_decode($_POST["idUsuarioProfFormula"]);
			@$nombreProfFormula = $utilidades->str_decode($_POST["nombreProfFormula"]);
			...
			@$num_mipress = $utilidades->str_decode(trim($_POST["num_mipress"]));
			@$num_ent_mipress = $utilidades->str_decode(trim($_POST["num_ent_mipress"]));
			@$num_poliza = $utilidades->str_decode(trim($_POST["num_poliza"]));
			...
			for ($i = 0; $i < $cant_productos; $i++) {
				...
				@$arr_pagos_detalle[$i]["cod_servicio_siesa"] = $utilidades->str_decode($_POST["cod_servicio_siesa_".$i]);
			...
			$arr_pagos_detalle_siesa = array();
			for ($i = 0; $i < $cant_productos; $i++) {
				if($utilidades->str_decode($_POST["valor_".$i]) > "0"){
				
				if($_POST["tipo_precio_".$i] == "P"){
					@$arr_pagos_detalle_siesa[$i]["cod_servicio"] = $utilidades->str_decode($_POST["cod_servicio_".$i]);
					
				}else if($_POST["tipo_precio_".$i] == "I"){
					@$arr_pagos_detalle_siesa[$i]["cod_servicio"] = $utilidades->str_decode($_POST["cod_servicio_siesa_".$i]);
				}
					
				@$arr_pagos_detalle_siesa[$i]["id_detalle_precio"] = $utilidades->str_decode($_POST["id_detalle_precio_".$i]);
				@$arr_pagos_detalle_siesa[$i]["tipo_precio"] = $utilidades->str_decode($_POST["tipo_precio_".$i]);
				@$arr_pagos_detalle_siesa[$i]["tipo_bilateral"] = $utilidades->str_decode($_POST["tipo_bilateral_".$i]);
				@$arr_pagos_detalle_siesa[$i]["num_autorizacion"] = $utilidades->str_decode($_POST["num_autorizacion_".$i]);
				@$arr_pagos_detalle_siesa[$i]["cantidad"] = $utilidades->str_decode($_POST["cantidad_".$i]);
				@$arr_pagos_detalle_siesa[$i]["valor"] = $utilidades->str_decode($_POST["valor_".$i]);
				@$arr_pagos_detalle_siesa[$i]["valor_cuota"] = $utilidades->str_decode($_POST["valor_cuota_".$i]);
			...
			if ($cant_productos > 0) {
				...
				//if ($ind_formato_valido) {
						...
						if ($_POST["tipoPago".$i] != "") {
							...
							if ($cant_anticipos_aux > 0) {
								for ($j = 0; $j < $cant_anticipos_aux; $j++) {
									...
									@$arr_anticipos[$j]["numero_anticipo"] = $utilidades->str_decode($_POST["num_anticipo_".$i."_".$j]);	
								}
					...
					if ($tipo_accion == 1 || $tipo_accion == 2) {
						...
						@$email = $utilidades->str_decode(trim($_POST["email"]));
						
						@$tipo_paciente = $utilidades->str_decode(trim($_POST["tipo_paciente"]));
						
						echo($num_factura."<br>".$num_pedido);
						...
						$id_paciente_resul = $dbPacientes->crear_editar_paciente($idPaciente, $id_tipo_documento, $numero_documento, $nombre_1,
											 ...
											 $telefono_1, $telefono_2, $id_usuario, $email,$tipo_paciente);
						...
						?><input type="hidden" id="hdd_id_paciente_resul" value="<?php echo($id_paciente_resul); ?>" /><?php

					} else {

						?><input type="hidden" id="hdd_id_paciente_resul" value="1" /><?php
					}
					...
					if ($ind_continuar) {
						...
						if ($tipo_accion == 1) {
							...
							$resultado = $dbPagos->registrarPagos("", "", $idPaciente, $id_usuario, "", "", "", "", "", "", "", "", "", $idPlan, $idConvenio,
										 "", "", $observaciones_pago, 1, $arr_pagos_detalle, array(), $idLugarCita, $idUsuarioProf, $idUsuarioProfFormula,$nombreProfFormula, $id_tercero_pago, "",$num_mipress,$num_ent_mipress,$num_poliza);
							...
							$resultado = $dbPagos->registrarPagos($idPago, $idAdmision, $idPaciente, $id_usuario, "", "", "", "", "", "", "", "", "", $idPlan, $idConvenio,
										 $num_factura, "", $observaciones_pago, 1, $arr_pagos_detalle, array(), $idLugarCita, $idUsuarioProf, $idUsuarioProfFormula, $nombreProfFormula, $id_tercero_pago, $num_pedido,$num_mipress,$num_ent_mipress,$num_poliza);
										
						}else if ($tipo_accion == 4) {
							//Edición de pagos registrados
							$resultado = $dbPagos->registrarPagos($idPago, $idAdmision, $idPaciente, $id_usuario, "", "", "", "", "", "", "", "", "", $idPlan, $idConvenio,
										 $num_factura, "", $observaciones_pago, 2, $arr_pagos_detalle, $arr_medios_pago, $idLugarCita, $idUsuarioProf,$idUsuarioProfFormula, $nombreProfFormula, $id_tercero_pago, $num_pedido,$num_mipress,$num_ent_mipress,$num_poliza);
						} else {
							//Registrar pago
							...
							//Si el medio de pago es diferente a contigencia y no es NP y además no tiene factura ni pedido, entonces haga el proceso de SIESA, de lo contario, registre el pago solo en base de datos.	
							if($arr_medios_pago[0]["tipoPago"] <> "100" && $arr_medios_pago[0]["tipoPago"] <> "99" 
								&& ($num_pedido_aut == "" || $num_factura_aut == "")){
								...
								if ($resultado_aux == 1) {
									...
									if($pago_obj["id_tercero"] != ""){
										...
										$id_tercero_pago = $pago_obj["id_tercero"];
										$tipo_doc_tercero = $tercero_obj["codigo_doc_siesa"];
										$num_doc_tercero = $tercero_obj["numero_documento"];

										$resultado_aux = $classClientesSIESA->crearTerceroClienteNoPaciente($id_tercero_pago, $compania,
															$num_doc_tercero, $idPaciente,$idLugarCita, $idAdmision, $id_usuario,$idPago);
					
									}else{

										$tercero_obj = $dbPacientes->getExistepaciente3($pago_obj["id_paciente"]);
										$id_tercero_pago = $pago_obj["id_paciente"];
										$tipo_doc_tercero = $tercero_obj["codigo_doc_siesa"];
										$num_doc_tercero = $tercero_obj["numero_documento"];

										
										$resultado_aux = $classClientesSIESA->crearTerceroClientePaciente($id_tercero_pago, $compania,
															$num_doc_tercero, $idPaciente,$idLugarCita, $idAdmision, $id_usuario,$idPago);
									...
									if ($resultado_aux == 1 && $ind_tipo_pago == "1") {
										...
										$resultado_aux = -1;
										$clienteSQLObj = $dbClientesSQL->obtenerClienteDetalle($convenio_obj["numero_documento"], $compania);
									
										if (!is_null($clienteSQLObj)) {
											$condicion_pago_entidad = $clienteSQLObj["Condicion_Pago"];
											$resultado_aux = 1;
										}
										?>
									...
									if ($resultado_aux == 1) {
										...
										if ($num_pedido_aut=="") {
											if ($ind_tipo_pago == "1") {
												//Se crea un pedido
												$resul_pedido = $classPedidosSIESAv2->crearPedido($compania, $co_ventas, $bodega_ventas, $usuario_obj["numero_documento"], $idPago, 
												$idConvenio, $idPlan, $idPaciente, $condicion_pago_entidad, $arr_pagos_detalle_siesa,$id_usuario);	
											...
											if ($resul_pedido > 0) {
												...
												?> 
													<input type="hidden" id="hdd_resul_pedido_siesa" name="hdd_resul_pedido_siesa" value="<?= $resul_pedido ?>" />
													<input type="hidden" id="hdd_num_pedido_siesa" name="hdd_num_pedido_siesa" value="<?= $num_pedido ?>" />
												<?php

												$result_entidad_doc=-1;
												$result_entidad_mov=-1;

												if ($num_pedido != "-1" && !is_null($num_pedido)) {
													
													if ($num_pedido != "" && $num_pedido > 0) {
															
														$result_entidad_doc = $classPedidosSiesa->crearEntidadPedidoDocumento($compania,$cod_prestador, $co_ventas, $bodega_ventas, $idPago, $num_pedido, 
														$id_usuario,$idConvenio, $idPlan,$idPaciente);
														
														if($result_entidad_doc ==1){
															//Se envían los números de autorización y carné correspondientes a SIESA
															$result_entidad_mov = $classPedidosSiesa->crearEntidadPedido($compania, $co_ventas, $bodega_ventas, $idPago, $num_pedido, $id_usuario); 
														}
														...
														if($result_entidad_doc == 1 && $result_entidad_mov == 1){
															...
															if($result_entidad_doc<>1){ 
																...
																$msg_pedido="Error en entidad dinámica del documento del pedido, ".$result_entidad_doc;

															}else if($result_entidad_mov<>1){ 

																$msg_pedido="Error en entidad dinámica del movimiento del pedido, ".$result_entidad_mov; 
															}
										...
										if ($num_factura_aut == "") {
											if (($ind_tipo_pago == "2" && $valor_total > 0) || $valor_copagos > 0) {
												if ($ind_tipo_pago == "1") {
													...
													switch ($tipo_precio_aux) {
														case "P": //Procedimientos
															...
															break;
														case "I": //Insumos
															$cod_servicio_max_aux = $detalle_aux["cod_servicio_siesa"];
													...
													$arr_productos_aux = $arr_pagos_detalle_siesa;
												}
												...
												$result_factura = $classFacturasSIESAv2->crearFactura($compania, $co_ventas, $bodega_ventas, $num_doc_tercero,
												$usuario_obj["numero_documento"], $idPago, $idConvenio, $idPlan, $ind_tipo_pago, $idPaciente,
												$arr_productos_aux, $arr_medios_pago, $arr_anticipos, $id_usuario);
												...
												$resul_entidad_factura_doc=-1;
												$resul_entidad_factura_mov=-1;
										
												if ($resul_factura == 1 && $num_factura != "-1") {
													?> 
														<input type="hidden" id="hdd_resul_factura_siesa" name="hdd_resul_factura_siesa" value="<?= $resul_factura ?>" />
														<input type="hidden" id="hdd_num_factura_siesa" name="hdd_num_factura_siesa" value="<?= $num_factura ?>" />
													<?php 	
														//Se crea la entidad dinámica de la factura.
														$resul_entidad_factura_doc = $classFacturasSiesa->crearEntidadFactura($compania, $cod_prestador, $co_ventas, $num_factura,
														$bodega_ventas,$arr_productos_aux, $num_doc_tercero, $usuario_obj["numero_documento"], $idPago, $idConvenio, $idPlan, $ind_tipo_pago, 
														$idPaciente, $id_usuario,1);
													...
													if($resul_entidad_factura_doc==1){
														...
														$bodega_ventas,$arr_productos_aux, $num_doc_tercero, $usuario_obj["numero_documento"], $idPago, $idConvenio, $idPlan, $ind_tipo_pago, 
														$idPaciente, $id_usuario,2);
													}
													
													if($resul_entidad_factura_doc==1 && $resul_entidad_factura_mov==1 && $result_factura == 1){
														
														//Si todo fue exitoso, se devuelve un mensaje exitoso.
														?> 
														...
														if($resul_entidad_factura_doc<>1){ 
															...
															$msg_factura="Error en entidad dinámica del documento de la factura, ".$resul_entidad_factura_doc; 

														} else if($resul_entidad_factura_mov<>1){ 

															$msg_factura="Error en entidad dinámica del movimiento de la factura, ".$resul_entidad_factura_mov; 
														}
														?> 
															<input type="hidden" id="hdd_resul_entidad_factura_siesa" name="hdd_resul_entidad_factura_siesa" value="-1" /> 
										...
										if ($resul_factura == 1 && $resul_pedido==1) {
											
											$resultado = $dbPagos->registrarPagos($idPago, $idAdmision, $idPaciente, $id_usuario, "", "", "", "", "", "", "", "", "", $idPlan, $idConvenio,
											$num_factura, $cmbEntidad, $observaciones_pago, 2, $arr_pagos_detalle, $arr_medios_pago, $idLugarCita, $idUsuarioProf,$idUsuarioProfFormula, 
											$nombreProfFormula, $id_tercero_pago,$num_pedido,$num_mipress,$num_ent_mipress,$num_poliza);
								...
								//Sí el medio de pago es NP 
								$resultado = $dbPagos->registrarPagos($idPago, $idAdmision, $idPaciente, $id_usuario, "", "", "", "", "", "", "", "", "", 
								$idPlan,$idConvenio,$num_factura,$cmbEntidad, $observaciones_pago, 2, $arr_pagos_detalle, $arr_medios_pago, $idLugarCita, 
								$idUsuarioProf, $idUsuarioProfFormula, $nombreProfFormula, $id_tercero_pago, $num_pedido,$num_mipress,$num_ent_mipress,$num_poliza);
								
							}							
				
						}
						
						?> <input type="hidden" id="hdd_id_pago_resul" value="<?= $resultado ?>" /> <?php
					}
				
			} else {
				?><input type="hidden" id="hdd_id_paciente_resul" value="-3" /><?php
			}
			break;
		...
		<div class="encabezado">
			<h3>Agregar consultas, procedimientos, medicamentos e insumos</h3>
		</div>  
		<div>
			<form id="frmListadoPrecios" name="frmListadoPrecios">
				<table style="width: 100%;">
					<tbody>
						...
						<tr>
							<td>
								<input type="text" id="txtParametroPrecios" name="txtParametroPrecios" placeholder="C&oacute;digo o nombre de la consulta, procedimiento, medicamento o insumo" onblur="trim_cadena(this);" />
							</td>
							<td style="width: 8%;">
							...
							<td style="width: 10%;">
								<input type="button" id="btn_buscar_usuario" nombre="btn_buscar_usuario" value="Ver todos" class="btnPrincipal peq" onclick="buscarProcedimientos(0, <?php echo($idcontador); ?>);" />
							</td>
						</tr>
					</tbody>
                </table>
		...
		case "5"://Imprime el resultado para el formulario flotante: Agregar Servicio
			...
			$rta_aux = $dbListasPrecios->getListasPreciosPorPlanParametro($idPlan, $parametroPrecios, "", 1);
		...
		<table class="paginated modal_table" style="width: 99%; margin: auto;">
			<thead>
				<tr>
                    ...
                    <th style="width:12%;">C&oacute;digo siesa</th>
					...
					foreach ($rta_aux as $value) {
						...
						switch ($value["tipo_precio"]) {
							...
							case "I":
								...
								$cod_insumo_sisa = $value["cod_insumo_siesa"];
								break;
						}
			...
			?>
			<tr onclick="agregar_servicio('<?php echo($cod_servicio_aux); ?>','<?php echo($cod_insumo_sisa); ?>', '<?php echo($texto_tipo_precio_aux); ?>', '<?php echo($nombre_servicio_aux); ?>', '<?php echo($value["valor"]); ?>', '<?php echo($valor_cuota_aux); ?>', '<?php echo($value["tipo_precio"]); ?>', <?php echo($value["tipo_bilateral"]); ?>, '<?php echo($texto_tipo_bilateral_aux); ?>', '<?php echo($value["ind_tipo_pago"]); ?>', '<?php echo($idContador); ?>');">
				<td><?php echo($cod_servicio_aux); ?></td>
                <td><?php echo($cod_insumo_sisa); ?></td>
		...
		case "11": //Borrar un pago
			...
			if ($pago_obj["estado_pago"] == "1" || $pago_obj["id_medio_pago"] == "99") {
				...
				$resultado = $dbPagos->borrar_pago($id_pago, $id_usuario, $observaciones_pago, $id_causal_borra, "");
			...
			<table border="0" style="width:100%;">
				...
				<tr>
					<td align="left" colspan="4">
						...
						<input type="hidden" id="hdd_num_anticipo_<?= $indice."_".$i ?>" name="hdd_num_anticipo_<?= $indice."_".$i ?>" value="<?= $anticipo_aux["num_anticipo"] ?>" />
		...
		case "15":
			...
			$num_pedido_ant == "" || $num_pedido_ant == "-1"  ? "-" : $num_pedido_ant;
			
			$num_factura_ant = "";
			$num_factura_ant = $classConsultasSiesa->consultarFacturaPago($compania, $id_pago);
			$num_factura_ant == "" || $num_factura_ant == "-1" ? "-" : $num_factura_ant;
		?>
		
        N&uacute;mero de factura: <?= $num_factura_ant ?>
		
		<input type="hidden" id="hdd_num_factura_siesa" value="<?= $num_factura_ant ?>">
        <br>
        N&uacute;mero de pedido: <?= $num_pedido_ant ?>
