...
//Esta función muestra la advertencia para registrar el pago
...
function validar_registrar_pago(id, tipo_accion) {
	...
	if($("#cmb_tipo_paciente").is(":visible")){
	   $("#cmb_tipo_paciente").removeClass("borde_error");
	}
	
	if($("#txt_profesional_formula").is(":visible")){
	   $("#txt_profesional_formula").removeClass("borde_error");
	}
	...
	if (tipo_accion != 3 && tipo_accion != 4 && tipo_accion != 5) {
		...
		if ($("#cmb_tipo_paciente").val() == "" && $("#cmb_tipo_paciente").is(":visible")) {
			$("#cmb_tipo_paciente").addClass("borde_error")
			ind_error = true;
		}
		
		if ($("#txt_profesional_formula").val().trim() == "" && $("#txt_profesional_formula").is(":visible")) {
			$("#txt_profesional_formula").addClass("borde_error")
			ind_error = true;
		}
...
var banderaClick=false;//Bandera en false, cuando se da click, el no permite hacer nuevamente un click, hasta que termine la operación.
...
function RegistrarPago(idContador, tipo_accion) {
	...
	if (!banderaClick){ //Validar de que solo se pueda hacer un click
		...
		var num_mipress = $("#txt_num_mipress").is(":visible") && !$('#txt_num_entrega_mipress').is(':disabled') ? $("#txt_num_mipress").val() : $("#hdd_num_mipress").val();
		var num_ent_mipress = $("#txt_num_entrega_mipress").is(":visible") && !$('#txt_num_entrega_mipress').is(':disabled') ? $("#txt_num_entrega_mipress").val() : $("#hdd_num_ent_mipress").val();
		var num_poliza = $("#txt_num_poliza").is(":visible") && !$('#txt_num_entrega_mipress').is(':disabled') ? $("#txt_num_poliza").val() : $("#hdd_num_poliza").val();
		...
		var params = "opcion=2&id_pago=" + $("#hdd_id_pago").val() +
					 ...
					 "&idUsuarioProfFormula=" + $("#cmb_usuario_prof_formula_" + idContador).val() +
					 "&nombreProfFormula=" + $("#txt_profesional_formula").val() +
					 ...
					 "&num_mipress=" + str_encode(num_mipress) +
					 "&num_poliza=" + str_encode(num_poliza) +
					 "&num_ent_mipress=" + str_encode(num_ent_mipress);
				...
				for (var j = 0; j < $("#hdd_cant_anticipos_" + i).val(); j++) {
					if ($("#chk_anticipo_" + i + "_" + j).is(":checked")) {
						params += "&id_anticipo_" + i + "_" + cont_anticipos_aux + "=" + $("#hdd_id_anticipo_" + i + "_" + j).val() +
									"&num_anticipo_" + i + "_" + cont_anticipos_aux + "=" + $("#hdd_num_anticipo_" + i + "_" + j).val() + 
		...
		for (i = 0; i < filas; i++) {
			if (isObject(document.getElementById("hdd_tipo_precio_" + i))) {
				params += "&id_detalle_precio_" + cont_productos + "=" + $("#hdd_detalle_precio_" + i).val() +
						  ...
						  "&cod_servicio_siesa_" + cont_productos + "=" + $("#hdd_cod_servicio_siesa_" + i).val() +
		...
		var tipo_paciente = $("#cmb_tipo_paciente").is(":visible") ? $("#cmb_tipo_paciente").val() : $("#hdd_tipo_paciente").val();
		if (tipo_accion == 1 || tipo_accion == 2) {
			params += "&id_tipo_documento=" + $("#cmb_tipo_documento").val() +
					  ...
					  "&tipo_paciente=" + str_encode(tipo_paciente) +
		...
		alert("Espera a que SIESA devuelva alguna respuesta.");
	}
}
					...
					$("#contenedor_error").html("Error en el registro externo del tercero del pago:<br />" + $("#hdd_mensaje_tercero_siesa").val());
						...
						if ($("#hdd_resul_tercero_entidad_siesa").length && $("#hdd_resul_tercero_entidad_siesa").val() != "1") {
							...
							if ($("#hdd_resul_factura_siesa").length && $("#hdd_resul_factura_siesa").val() != "1") {
								$("#contenedor_error").html("Error en el registro externo de la factura asociada al pago <br />");
						}else{
								if ($("#hdd_resul_entidad_factura_siesa").length && $("#hdd_resul_entidad_factura_siesa").val() != "1") {
									$("#contenedor_error").html("Error en el registro externo de la entidad dinamica de la factura asociada al pago <br />" + $("#hdd_mensaje_entidad_factura_siesa").val());
							}else {
									...
									if ($("#hdd_resul_pedido_entidad_siesa").length && $("#hdd_resul_pedido_entidad_siesa").val() != "1") {
										$("#contenedor_error").html("Error en el registro de la entidad dinamica del pedido <br />" + $("#hdd_mensaje_pedido_entidad_siesa").val());
...
function agregar_servicio(codigo,codigo_siesa, tipo, nombre, precio, precioCuota, tipoPrecio, tipoBilateral, nombreBilateral, ind_tipo_pago, idContador) {
	...
	var cod_item_agregar = "";
	if(tipoPrecio == "P"){
		cod_item_agregar = codigo;		
	}else if(tipoPrecio == "I"){
		cod_item_agregar = codigo_siesa;
	}
	...
	$("#tablaPrecios" + idContador).append(
		'<tr id="tr_producto_' + indice + '">' +
			'<td align="center">' +
				...
				'<input type="hidden" name="hdd_cod_servicio_siesa_' + indice + '" id="hdd_cod_servicio_siesa_' + indice + '" value="' + codigo_siesa + '" />' +
...
function seleccionarProfesional() {
	...
	if($("#cmb_usuario_prof_formula_1").val() == "234"){
		$(".td_profesional_formula").css("display","table-cell");	
	}else{
		$(".td_profesional_formula").css("display","none");
		$("#txt_profesional_formula").val("");
	}	
}
...
function actualizar_ind_num_aut() {
	...
	//Se muestran o se ocultan los campos de mipress dependiendo del convenio seleccionado.
	if($("#hdd_ind_num_aut").val() == 1){
