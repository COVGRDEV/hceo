	...
	switch ($tipoReporte) {
		case "1": //Reporte general
			...
			if($diferencia_dias >= 34){
				...
				$objPHPExcel->setActiveSheetIndex(0)->getColumnDimension("U")->setWidth(30);
				$objPHPExcel->setActiveSheetIndex(0)->getColumnDimension("V")->setWidth(30);
				$objPHPExcel->setActiveSheetIndex(0)->getColumnDimension("W")->setWidth(30);
				...
				$objPHPExcel->setActiveSheetIndex(0)->getColumnDimension("Z")->setWidth(40);
				...
				$objPHPExcel->getActiveSheet()->mergeCells("A".$contador_linea.":"."H".$contador_linea);
				$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(0, $contador_linea, "OJO: EL REPORTE DE MERCADEO CONTIENE LA INFORMACIÓN DE TODOS LOS PACIENTES, A SU VEZ SE INDICA AQUELLOS QUE TIENEN EL INDICADOR HABEAS DATA Y QUIENES NO.");

				$contador_linea = $contador_linea + 2;
				$objPHPExcel->getActiveSheet()
							...
							->setCellValue("J".$contador_linea, "TELÉFONO 2")
							->setCellValue("K".$contador_linea, "OCUPACIÓN")
							->setCellValue("L".$contador_linea, "EDAD")
							->setCellValue("M".$contador_linea, "PAIS")
							->setCellValue("N".$contador_linea, "DEPARTAMENTO")
							->setCellValue("O".$contador_linea, "MUNICIPIO")
							->setCellValue("P".$contador_linea, "CATEGORIA MERCADEO")
							->setCellValue("Q".$contador_linea, "SUBCATEGORIA MERCADEO")
							->setCellValue("R".$contador_linea, "CONVENIO")
							->setCellValue("S".$contador_linea, "PLAN")
							->setCellValue("T".$contador_linea, "NOMBRE DEL PROFESIONAL")
							->setCellValue("U".$contador_linea, "MOTIVO DE LA CITA")
							->setCellValue("V".$contador_linea, "GENERO")
							->setCellValue("W".$contador_linea, "ESTADO CIVIL ")
							->setCellValue("X".$contador_linea, "ZONA")
							->setCellValue("Y".$contador_linea, "DESPLAZADO")
							->setCellValue("Z".$contador_linea, "HABEAS DATA");;
											
				$objPHPExcel->getActiveSheet()->getStyle("A".$contador_linea.":Z".$contador_linea)->getFont()->setBold(true);
				...
				foreach ($lista_mercadeo as $reporte_mercadeo) {
					...
					if (is_null($reporte_mercadeo["subcategoria"])){
						...
						}else if(!is_null($reporte_mercadeo["otro"])){
							$nom_subc_aux = $reporte_mercadeo["otro"];
					...
					if($reporte_mercadeo["ind_habeas_data"] == 1){
						cellColor("Z".$contador_linea, 'D9F2FF');
					...
					$objPHPExcel->getActiveSheet()
						...
						->setCellValue("J".$contador_linea, $reporte_mercadeo["telefono_2"])
						->setCellValue("K".$contador_linea, $reporte_mercadeo["profesion"])
						->setCellValue("L".$contador_linea,	$edad_aux)
						->setCellValue("M".$contador_linea, $reporte_mercadeo["pais"])
						->setCellValue("N".$contador_linea, $nom_dep_aux)	
						->setCellValue("O".$contador_linea, $nom_mun_aux)
						->setCellValue("P".$contador_linea, $reporte_mercadeo["nom_categoria"])
						->setCellValue("Q".$contador_linea, $nom_subc_aux)
						->setCellValue("R".$contador_linea, $reporte_mercadeo["nombre_convenio"])	
						->setCellValue("S".$contador_linea, $reporte_mercadeo["nombre_plan"])	
						->setCellValue("T".$contador_linea, $reporte_mercadeo["nombre_usuario_prof"])
						->setCellValue("U".$contador_linea, $reporte_mercadeo["nombre_tipo_cita"])
						->setCellValue("V".$contador_linea, $reporte_mercadeo["genero"])
						->setCellValue("W".$contador_linea, $reporte_mercadeo["estado_civil"])
						->setCellValue("X".$contador_linea, $reporte_mercadeo["zona"])
						->setCellValue("Y".$contador_linea, $reporte_mercadeo["desplazado"])
						->setCellValue("Z".$contador_linea, $mensaje);
						$contador_linea++;
