	...
	switch ($tipoReporte) {
		case "1": //Reporte tiempos de atención 
			...
			foreach ($rta_atenciones as $indice => $value) { 
				...
				$subindice = $value["nombre_convenio"]."-".$value["nombre_plan"]."-".$value["orden_estado"]."-".$value["nombre_estado"]."-".$value["numero_documento"]; 
			...
			// Ordernar vector de duraciones, e imprimir en celdas del excel: 
				...
				foreach ($vec_tiempos as $clave=>$valor) {				
					...
					$aux_porcentaje = $indicador;
					$aux_atenciones = $atenciones_acumuladas;
					$aux_valor = $valor;
