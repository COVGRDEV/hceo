...
class DbAsignarCitas extends DbConexion {
    ...
    public function getListaCitasFecha($id_usuario, $fecha) {
        try {
            $sql = "SELECT c.*, tc.nombre_tipo_cita, e.nombre_convenio,
					DATE_ADD(c.fecha_cita, INTERVAL c.tiempo_cita MINUTE) AS fecha_fin_cita
					FROM citas c
					INNER JOIN tiempos_citas_prof t ON t.id_tipo_cita=c.id_tipo_cita AND c.id_usuario_prof=t.id_usuario
					INNER JOIN tipos_citas tc ON tc.id_tipo_cita=t.id_tipo_cita
    ...
    public function crearCita($txt_primer_nombre, $txt_segundo_nombre, $txt_primer_apellido, $txt_segundo_apellido, $cmb_tipo_documento,
			...
			$tiempo_cita, $hdd_id_usuario, $id_usuario_crea, $hdd_id_paciente, $hdd_lugar_cita, $ind_reasignar, $id_cita_reasignar,
			$chk_no_pago, $id_prog_cx,$sexo="") {

        try {
			$hdd_id_paciente == "" ?  $hdd_id_paciente = "NULL" : $hdd_id_paciente;
          	intval($id_cita_reasignar) == 0 ? $id_cita_reasignar = "NULL" : $id_cita_reasignar; 
			$id_prog_cx == "" ? $id_prog_cx = "NULL" :  $id_prog_cx; 
           	$sexo == "" ? $sexo = "NULL" : $sexo;
            ...
            $sql = "CALL pa_crear_cita('".$txt_primer_nombre."', '".$txt_segundo_nombre."', '".$txt_primer_apellido."', '".$txt_segundo_apellido."', ".
                    ...
                    $hdd_lugar_cita.", ".$ind_reasignar.", ".$id_cita_reasignar.", ".$id_prog_cx.", ".$id_usuario_crea.",".$sexo.", @id)";
    ...
    public function actualizarCita($txt_primer_nombre, $txt_segundo_nombre, $txt_primer_apellido, $txt_segundo_apellido, $cmb_tipo_documento,
			$txt_numero_documento, $txt_numero_telefono, $cmb_convenio, $cmb_lentes, $txt_observacion_cita, $id_usuario_edita, $hdd_id_cita,
			$hdd_lugar_cita, $chk_no_pago, $hdd_id_paciente, $sexo="") {
        try {
			$hdd_id_paciente == '' ? $hdd_id_paciente = "NULL" : $hdd_id_paciente;
			$sexo == '' ? $sexo = "NULL" : $sexo;
          
            $sql = "CALL pa_editar_cita('".$txt_primer_nombre."', '".$txt_segundo_nombre."', '".$txt_primer_apellido."', '".$txt_segundo_apellido."', ".			$cmb_tipo_documento.", '".$txt_numero_documento."', "."'".$txt_numero_telefono."', ".$cmb_convenio.", ".$cmb_lentes.", ".
			$chk_no_pago.", '".$txt_observacion_cita."', ".$hdd_id_paciente.", ".$id_usuario_edita.", ".$hdd_id_cita.", ".$hdd_lugar_cita.", ".$sexo.", @id)";
    ...
    public function getCita($id_cita) {
        try {
            $sql = "SELECT C.*, DATE_FORMAT(C.fecha_nacimiento, '%d/%m/%Y') AS fecha_nac, TC.nombre_tipo_cita,
					U.nombre_usuario, U.apellido_usuario, DATE_FORMAT(C.fecha_crea, '%d/%m/%Y %h:%i:%s %p') AS fecha_crea_t,
					UC.nombre_usuario AS nombre_confirma, UC.apellido_usuario AS apellido_confirma,
					DATE_FORMAT(C.fecha_confirma, '%d/%m/%Y %h:%i:%s %p') AS fecha_confirma_t, EC.nombre_detalle AS estado_cita, lc.nombre_detalle as nombre_lugar_cita
					FROM citas C
					...
					INNER JOIN listas_detalle lc ON lc.id_detalle = c.id_lugar_cita
    ...
    public function getBuscarDocumentoCita($txt_buscar, $id_cita, $fecha, $tipo_accion) {
        try {
            $sql = "SELECT * FROM citas
					...
					AND fecha_cita BETWEEN '".$fecha."' AND DATE_ADD('".$fecha."', INTERVAL 1 DAY)
    ...
    public function getBuscarCitasPersonas($txt_buscar, $id_usuario) {
        try {
            $sql = "SELECT c.*, u.id_usuario, u.nombre_usuario, u.apellido_usuario, tc.nombre_tipo_cita, ld.id_detalle, ld.id_lista, ld.codigo_detalle, ld.nombre_detalle, ld.orden
					FROM citas c
					...
					AND c.fecha_cita>=(CURDATE() - INTERVAL 2 MONTH)
