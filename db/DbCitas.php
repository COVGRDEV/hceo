<?php
	...
	class DbCitas extends DbConexion {
		...
		public function getEstadoagendados2() {
			try {
				$sql = "SELECT c.nombre_1, c.nombre_2, c.apellido_1, c.apellido_2, DATE_FORMAT(c.fecha_cita, '%H:%i') AS fecha_cita, u.nombre_usuario,
						u.apellido_usuario, c.id_cita, c.id_paciente, c.numero_documento, ld.nombre_detalle, cv.nombre_convenio,
						DATE_FORMAT(c.fecha_cita, '%h:%i %p') AS hora_cita, c.observacion_cita, c.fecha_llegada, c.fecha_ingreso,
						DATE_FORMAT(c.fecha_llegada,'%h:%i %p') AS fecha_llegada2_aux, DATE_FORMAT(c.fecha_ingreso,'%h:%i %p') AS fecha_ingreso2_aux 
						FROM citas c
		...
		public function getCita($id_cita) {
			try {
				$sql = "SELECT c.*, tc.nombre_tipo_cita AS nombre_tipo_cita, CONCAT(u.nombre_usuario,' ',u.apellido_usuario) AS profesional_atiende,
							DATE_FORMAT(c.fecha_cita, '%d/%m/%Y') AS fecha_consulta, DATE_FORMAT(c.fecha_cita, '%H:%i') AS hora_consulta,
							DATE_FORMAT(fecha_nacimiento, '%d/%m/%Y') AS fecha_nacimiento_aux, c.nombre_2 AS nombre2, ld.nombre_detalle,
							CONCAT(up.nombre_usuario, ' ', up.apellido_usuario) AS usuario_creador, DATE_FORMAT(c.fecha_crea, '%d/%m/%Y %H:%i:%s') AS fecha_creacion_aux,
							cv.nombre_convenio AS nombre_convenio_aux, DATE_FORMAT(c.fecha_llegada,'%h:%i %p') AS fecha_llegada2_aux,
							DATE_FORMAT(c.fecha_ingreso,'%h:%i %p') AS fecha_ingreso2_aux, cv.ind_eco, DATE_FORMAT(c.fecha_cita, '%h:%i %p') AS hora_consulta_t,
							sd.dir_sede_det AS lugar_cita,ec.nombre_detalle AS nombre_estado_cita
							FROM citas c
							...
							INNER JOIN sedes_det sd ON sd.id_detalle=c.id_lugar_cita
		...
		public function getBuscarpacientes($parametro) {//Busca pacientes por numero de identificacion o nombre
			try {
				...
				$sql = "SELECT 0 AS id_admision, C.id_cita, CASE C.id_estado_cita WHEN 14 THEN 1 ELSE 10 END AS id_estado_atencion,
						...
						WHERE A.fecha_admision BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL 1 DAY)
		...
		public function getBuscarPacientesAnt($parametro, $num_dias) {//Busca pacientes por numero de identificacion o nombre
			try {
				...
				$sql = "SELECT 0 AS id_admision, C.id_cita, CASE C.id_estado_cita WHEN 14 THEN 1 ELSE 10 END AS id_estado_atencion,
						...
						C.nombre_1, C.nombre_2, C.apellido_1, C.apellido_2, CONCAT(U.nombre_usuario, ' ', U.apellido_usuario) AS nombre_usuario,
						C.fecha_cita, DATE_FORMAT(C.fecha_cita, '%H:%i %p') AS tiempo_t, DATE_FORMAT(C.fecha_cita, '%d/%m/%Y') AS fecha_cita_aux,
						TD.codigo_detalle AS nombre_detalle2, C.numero_documento, C.id_paciente, C.id_tipo_cita, C.id_lugar_cita,
						...
						INNER JOIN usuarios U ON C.id_usuario_prof=U.id_usuario
						LEFT JOIN listas_detalle LC ON C.id_lugar_cita=LC.id_detalle
						WHERE C.fecha_cita BETWEEN DATE_SUB(CURDATE(), INTERVAL ".$num_dias." DAY) AND DATE_ADD(CURDATE(), INTERVAL 1 DAY)
						AND (CONCAT(C.nombre_1, ' ', IFNULL(C.nombre_2, ''), ' ', C.apellido_1, ' ', IFNULL(C.apellido_2, '')) LIKE '%".$parametro."%'
						OR C.numero_documento LIKE '%".$parametro."%')
						...
						INNER JOIN listas_detalle LD ON P.id_tipo_documento=LD.id_detalle
						LEFT JOIN listas_detalle LC ON A.id_lugar_cita=LC.id_detalle
						WHERE A.fecha_admision BETWEEN DATE_SUB(CURDATE(), INTERVAL ".$num_dias." DAY) AND DATE_ADD(CURDATE(), INTERVAL 1 DAY)
						AND (CONCAT(P.nombre_1, ' ', IFNULL(P.nombre_2, ''), ' ', P.apellido_1, ' ', IFNULL(P.apellido_2, '')) LIKE '%".$parametro."%'
						OR P.numero_documento LIKE '%".$parametro."%')
						...
						INNER JOIN listas_detalle LD ON P.id_tipo_documento=LD.id_detalle
						LEFT JOIN listas_detalle LC ON PA.id_lugar_cita=LC.id_detalle
						WHERE PA.fecha_crea BETWEEN DATE_SUB(CURDATE(), INTERVAL ".$num_dias." DAY) AND DATE_ADD(CURDATE(), INTERVAL 1 DAY)
						AND (CONCAT(P.nombre_1, ' ', IFNULL(P.nombre_2, ''), ' ', P.apellido_1, ' ', IFNULL(P.apellido_2, '')) LIKE '%".$parametro."%'
						OR P.numero_documento LIKE '%".$parametro."%')
						AND PA.estado_pago=1
						AND PA.id_admision IS NULL
		...
		public function getReporteCitas($profesional, $lugar, $fechaInicial, $fechaFinal, $cita, $limitado, $tipoCita, $hora, $tiposCitas, $convenio, $ind_conteo = 0) {
			try {
				...
				if ($fechaInicial != '' && $fechaFinal != '') {
					$concatenacion .= $conector . "C.fecha_cita BETWEEN '".$fechaInicial."' AND DATE_ADD('".$fechaFinal."', INTERVAL 1 DAY) ";
					$conector = "AND ";
				} else if ($fechaInicial != '') {
					$concatenacion .= $conector . "C.fecha_cita>='".$fechaInicial."' ";
					$conector = "AND ";
				} else if ($fechaFinal != '') {
					$concatenacion .= $conector . "C.fecha_cita<=DATE_ADD('".$fechaFinal."', INTERVAL 1 DAY) ";
					$conector = "AND ";
				}
				if ($cita != '') {
				...
				if ($ind_conteo != 1) {
					...
					$sql = "SELECT C.*, DATE_FORMAT(C.fecha_cita, '%H:%i') AS hora_aux, CONCAT(U.nombre_usuario, ' ', U.apellido_usuario) AS profesional_aux,
							TC.nombre_tipo_cita, LD.nombre_detalle, CV.nombre_convenio AS nombre_convenio_aux, U.nombre_usuario, U.apellido_usuario,
							LC.nombre_detalle AS lugar_cita,   CONCAT(UC.nombre_usuario,' ', UC.apellido_usuario) AS nombre_usuario_crea, 
							  DATE_FORMAT(C.fecha_crea, '%d/%m/%Y') AS fecha_crea_t,DATE_FORMAT(C.fecha_crea, '%h:%i %p') AS hora_crea_t,
							DATE_FORMAT(C.fecha_cita, '%d/%m/%Y') AS fecha_cita_t, DATE_FORMAT(C.fecha_cita, '%h:%i %p') AS hora_cita_t,
		...
		public function buscarEstadoagendados($estado_cita, $parametro) {
			try {
				...
				$sql = "SELECT c.nombre_1, c.nombre_2, c.apellido_1, c.apellido_2, DATE_FORMAT(c.fecha_cita, '%H:%i') AS fecha_cita,
						u.nombre_usuario, u.apellido_usuario, c.id_cita, c.id_paciente, c.numero_documento, ld.nombre_detalle,
						cv.nombre_convenio, DATE_FORMAT(c.fecha_cita, '%h:%i %p') AS hora_cita, c.observacion_cita, c.fecha_llegada,
						...
						INNER JOIN convenios cv ON cv.id_convenio=c.id_convenio
						WHERE c.fecha_cita BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL 1 DAY)
						AND c.id_estado_cita IN (".$estado_cita.", 130, 17)
						AND (CONCAT(c.nombre_1, ' ', c.nombre_2, ' ', c.apellido_1, ' ', c.apellido_2) LIKE '%".$parametro."%'
						OR c.numero_documento LIKE '%".$parametro."%'
		...
		public function get_lista_citas_cambio_estado($parametro) {
			try {
				...
				$sql = "SELECT C.*, TD.codigo_detalle AS cod_tipo_documento, TD.nombre_detalle AS tipo_documento,
						...
						AND C.fecha_cita BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL 1 DAY)
