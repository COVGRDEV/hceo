<?php
	...
	class DbConsultaOptometria extends DbHistoriaClinica {
		...
		public function getConsultaOptometria($id_hc) {
	        try {
	            $sql = "SELECT O.*, DATE_FORMAT(HC.fecha_hora_hc, '%d/%m/%Y') AS fecha_hc_t, DM.nombre_detalle AS dominancia_ocular, 
						lstp.nombre_detalle AS slct_lente, lstf.nombre_detalle AS slct_filtro, lstv.nombre_detalle AS tiempo_vigencia, lstm.nombre_detalle AS periodo,
						LC.nombre_detalle AS lugar_cita, SD.tel_sede_det , SD.dir_sede_det, SD.email_sede_det, 
						sblod.nombre_detalle AS agudezaL_ojoD, sbcod.nombre_detalle AS agudezaC_ojoD, sbloi.nombre_detalle AS agudezaL_ojoI, sbcoi.nombre_detalle AS agudezaL_ojoI
						FROM consultas_optometria O
						LEFT JOIN historia_clinica HC ON O.id_hc=HC.id_hc
		...
		public function EditarConsultaOptometria($hdd_id_hc_consulta, $hdd_id_admision, $txt_anamnesis,
				...
				$subjetivo_esfera_od, $subjetivo_cilindro_od, $subjetivo_eje_od, $subjetivo_lejos_od, $subjetivo_media_od, $subjetivo_ph_od, $subjetivo_adicion_od, $subjetivo_cerca_od,
				$subjetivo_esfera_oi, $subjetivo_cilindro_oi, $subjetivo_eje_oi, $subjetivo_lejos_oi, $subjetivo_media_oi, $subjetivo_ph_oi, $subjetivo_adicion_oi, $subjetivo_cerca_oi,
				...
				$id_usuario_crea, $tipo_guardar, $presion_intraocular_od, $presion_intraocular_oi, $diagnostico_optometria, $array_diagnosticos,
				$txt_observaciones_subjetivo, $txt_observaciones_rxfinal, $cmb_validar_consulta, $cmb_examinado_antes, $rx_gafas,
				$rx_lc, $rx_refractiva, $rx_ayudas_bv, $array_gafas, $array_ldc, $array_cxr, $array_ayudas_bv, $array_lenso, $alternativa_anteojos, $alternativa_lc, $alternativa_refractiva, $alternativa_otra,
				$cmb_paciente_dilatado, $observaciones_optometricas_finales, $cmb_dominancia_ocular, $observaciones_admision, $nombre_usuario_alt,
				$tipo_lente, $tipos_lentes_slct,$tipo_filtro_slct, $tiempo_vigencia_slct, $tiempo_periodo, $distancia_pupilar, $form_cantidad, $txt_observaciones_optometria) {									
			try {
				...
				// Procesar gafas: 
				$sql_delete_detalle_tmp = "DELETE FROM consultas_optometria_gafas_tmp WHERE id_hc = ".$hdd_id_hc_consulta." AND id_usuario_crea = ".$id_usuario_crea;
				$arrCampos_delete[0] = "@id";
				if($this->ejecutarSentencia($sql_delete_detalle_tmp, $arrCampos_delete)){
					...
					foreach($array_gafas as $fila_detalle){						
						...
						if ($tipo_lente2 == "") { $tipo_lente2 = "NULL"; }						
						if ($tipo_lente_det == "") { $tipo_lente_det = "NULL"; }
						if ($tipo_lente_otro == "") { $tipo_lente_otro = ""; }
						if ($tipo_filtro == "") { $tipo_filtro = "NULL"; }
						if ($tiempo_rx == "") { $tiempo_rx = "NULL"; }
						if ($und_tiempo_rx == "") { $und_tiempo_rx = "NULL"; }
						if ($grado_satisfacc == "") { $grado_satisfacc= "NULL"; } 			
						
						if ($lenso_esfera_od == "") { $lenso_esfera_od = ""; }
						if ($lenso_cilindro_od == "") { $lenso_cilindro_od = ""; }
						if ($lenso_eje_od == "") { $lenso_eje_od = ""; }
						if ($lenso_adicion_od == "") { $lenso_adicion_od = ""; }
						if ($lenso_lejos_od == "") { $lenso_lejos_od = "NULL"; }
						if ($lenso_media_od == "") { $lenso_media_od = "NULL"; }
						if ($lenso_cerca_od == "") { $lenso_cerca_od = "NULL"; }						
						if ($lenso_esfera_oi == "") { $lenso_esfera_oi = ""; }
						if ($lenso_cilindro_oi == "") { $lenso_cilindro_oi = ""; }
						if ($lenso_eje_oi == "") { $lenso_eje_oi = ""; }
						if ($lenso_adicion_oi == "") { $lenso_adicion_oi = ""; }
						if ($lenso_lejos_oi == "") { $lenso_lejos_oi = "NULL"; }
						if ($lenso_media_oi == "") { $lenso_media_oi = "NULL"; }
						if ($lenso_cerca_oi == "") { $lenso_cerca_oi = "NULL"; }						
						
						if ($prisma_tipo_od == "") { $prisma_tipo_od = "NULL"; } 
						if ($prisma_potencia_od == "") { $prisma_potencia_od = "NULL"; } 
						if ($prisma_base_od == "") { $prisma_base_od = "NULL"; } 
						if ($prisma_tipo_oi == "") { $prisma_tipo_oi = "NULL"; } 
						if ($prisma_potencia_oi == "") { $prisma_potencia_oi = "NULL"; } 
						if ($prisma_base_oi == "") { $prisma_base_oi = "NULL"; } 
						
						if ($ind_presentes == "") { $ind_presentes = "1"; }
						...
						$sql_insert_detalle="INSERT INTO consultas_optometria_gafas_tmp (
												...
												prisma_tipo_oi, prisma_potencia_oi, prisma_base_oi, 
												id_usuario_crea 
											) VALUES ($hdd_id_hc_consulta, $ind_presentes, $tipo_lente2, $tipo_lente_det, '$tipo_lente_otro', $tipo_filtro, 
												$tiempo_rx, $und_tiempo_rx, $grado_satisfacc, 
												'$lenso_esfera_od', '$lenso_cilindro_od', '$lenso_eje_od', '$lenso_adicion_od', $lenso_lejos_od, $lenso_media_od, $lenso_cerca_od, 
												'$lenso_esfera_oi', '$lenso_cilindro_oi', '$lenso_eje_oi', '$lenso_adicion_oi', $lenso_lejos_oi, $lenso_media_oi, $lenso_cerca_oi, 												
												$prisma_tipo_od, $prisma_potencia_od, $prisma_base_od, 
												$prisma_tipo_oi, $prisma_potencia_oi, $prisma_base_oi, 
				...
				$sql_delete_detalle_tmp = "DELETE FROM consultas_optometria_ldc_tmp WHERE id_hc = ".$hdd_id_hc_consulta." AND id_usuario_crea = ".$id_usuario_crea;
				$arrCampos_delete[0] = "@id";
				if($this->ejecutarSentencia($sql_delete_detalle_tmp, $arrCampos_delete)){
					$j=1;
					foreach($array_ldc as $fila_detalle){ 
						...
						if ($tipo_ldc == "") { $tipo_ldc = "NULL"; } 
						if ($tipo_ldc_det == "") { $tipo_ldc_det = "NULL"; } 
						if ($disenio == "") { $disenio= "NULL"; } 
						if ($tipo_duracion == "") { $tipo_duracion = "NULL"; } 						
						if ($ojo == "") { $ojo = "NULL"; } 
						if ($tiempo_uso == "") { $tiempo_uso = "NULL"; } 
						if ($und_tiempo_uso == "") { $und_tiempo_uso= "NULL"; } 
						if ($tiempo_no_uso == "") { $tiempo_no_uso= "NULL"; } 
						if ($und_tiempo_no_uso == "") { $und_tiempo_no_uso = "NULL"; } 
						if ($modalidad_uso == "") { $modalidad_uso = "NULL"; } 
						if ($tiempo_reemplazo == "") { $tiempo_reemplazo= "NULL"; } 
						if ($grado_satisfacc == "") { $grado_satisfacc= "NULL"; } 
						if ($tipo_ldc_otro == "") { $tipo_ldc_otro= ""; } 
						if ($tipo_duracion_det == "") { $tipo_duracion_det = "NULL"; } 
						if ($ind_presentes == "") { $ind_presentes = "1"; } 

						$sql_insert_detalle="INSERT INTO consultas_optometria_ldc_tmp (id_hc, ind_presentes, tipo_ldc, tipo_ldc_det, tipo_ldc_otro, disenio, tipo_duracion, tipo_duracion_det, ojo, tiempo_uso, und_tiempo_uso, tiempo_no_uso, und_tiempo_no_uso, 
												modalidad_uso, tiempo_reemplazo, grado_satisfacc, id_usuario_crea 
											) VALUES ($hdd_id_hc_consulta, $ind_presentes, $tipo_ldc, $tipo_ldc_det, '$tipo_ldc_otro', $disenio, $tipo_duracion, $tipo_duracion_det, $ojo, $tiempo_uso, $und_tiempo_uso, $tiempo_no_uso, $und_tiempo_no_uso, 
												$modalidad_uso, $tiempo_reemplazo, $grado_satisfacc, $id_usuario_crea 
											)"; 
				...
				$sql_delete_detalle_tmp = "DELETE FROM consultas_optometria_ayudas_bv_tmp WHERE id_hc = ".$hdd_id_hc_consulta." AND id_usuario_crea = ".$id_usuario_crea;
				$arrCampos_delete[0] = "@id";
				if($this->ejecutarSentencia($sql_delete_detalle_tmp, $arrCampos_delete)){
					$j=1;
					foreach($array_ayudas_bv as $fila_detalle){ 			
						...
						if ($tipo_abv == "") { $tipo_abv = "NULL"; } 
						if ($tipo_abv_det == "") { $tipo_abv_det = "NULL"; } 
						if ($grado_satisfacc == "") { $grado_satisfacc= "NULL"; } 
						if ($tipo_abv_otro == "") { $tipo_abv_otro = ""; } 
						if ($ind_presentes == "") { $ind_presentes = "1"; }
						
						$sql_insert_detalle="INSERT INTO consultas_optometria_ayudas_bv_tmp (id_hc, ind_presentes, tipo_abv, tipo_abv_det, tipo_abv_otro, grado_satisfacc, id_usuario_crea) 
											VALUES ($hdd_id_hc_consulta, $ind_presentes, $tipo_abv, $tipo_abv_det, '$tipo_abv_otro', $grado_satisfacc, $id_usuario_crea)"; 
				...
				$sql_delete_detalle_tmp = "DELETE FROM consultas_optometria_cxr_tmp WHERE id_hc = ".$hdd_id_hc_consulta." AND id_usuario_crea = ".$id_usuario_crea;
				$arrCampos_delete[0] = "@id";
				if($this->ejecutarSentencia($sql_delete_detalle_tmp, $arrCampos_delete)){
					$j=1;
					foreach($array_cxr as $fila_detalle){ 			
						...
						if ($tipo_cxr == "") { $tipo_cxr = "NULL"; } 
						if ($tipo_cxr_det == "") { $tipo_cxr_det = "NULL"; } 						
						if ($ind_ajuste == "") { $ind_ajuste = "0"; } 
						if ($ojo == "") { $ojo = "NULL"; } 
						if ($tiempo_uso == "") { $tiempo_uso = "NULL"; } 
						if ($und_tiempo_uso == "") { $und_tiempo_uso = "NULL"; } 
						if ($entidad == "") { $entidad = "NULL"; } 						
						if ($id_usuario_cirujano == "") { $id_usuario_cirujano = "NULL"; } 
						if ($cirujano_otro == "") { $cirujano_otro = ""; } 
						if ($grado_satisfacc == "") { $grado_satisfacc = "NULL"; } 
						if ($tipo_cxr_otro == "") { $tipo_cxr_otro = ""; } 
						
						$sql_insert_detalle="INSERT INTO consultas_optometria_cxr_tmp (id_hc, tipo_cxr, tipo_cxr_det, tipo_cxr_otro, ind_ajuste, ojo, tiempo_uso, und_tiempo_uso, 
												entidad, id_usuario_cirujano, cirujano_otro, grado_satisfacc, id_usuario_crea 
											) VALUES ($hdd_id_hc_consulta, $tipo_cxr, $tipo_cxr_det, '$tipo_cxr_otro', $ind_ajuste, $ojo, $tiempo_uso, $und_tiempo_uso, 
												$entidad, $id_usuario_cirujano, '$cirujano_otro', $grado_satisfacc, $id_usuario_crea)"; 
				...
				$sql = "CALL pa_editar_consultas_optometria(".$hdd_id_hc_consulta.", ".$hdd_id_admision.", '".$txt_anamnesis."', ".
						$avsc_lejos_od.", ".$avsc_media_od.", ".$avsc_cerca_od.", ".$avsc_lejos_oi.", ".$avsc_media_oi.", ".$avsc_cerca_oi.", '".
						$querato_k1_od."', '".$querato_ejek1_od."', '".$querato_dif_od."', '".$querato_k1_oi."', '".$querato_ejek1_oi."', '".$querato_dif_oi."', '".
						$refraobj_esfera_od."', '".$refraobj_cilindro_od."', '".$refraobj_eje_od."', ".$refraobj_lejos_od.", '".
						$refraobj_esfera_oi."', '".$refraobj_cilindro_oi."', '".$refraobj_eje_oi."', ".$refraobj_lejos_oi.", '".
						...
						$txt_observaciones_subjetivo."', '".$txt_observaciones_rxfinal."', ".$cmb_validar_consulta.", ".$cmb_examinado_antes.", ".$rx_gafas.", ".
						$rx_lc.", ".$rx_refractiva.", ".$rx_ayudas_bv.", ".$alternativa_anteojos.", ".$alternativa_lc.", ".$alternativa_refractiva.", '".$alternativa_otra."', ".
						$cmb_paciente_dilatado.", '".$observaciones_optometricas_finales."', ".$cmb_dominancia_ocular.", ".$observaciones_admision.", ".
						$nombre_usuario_alt.", ".$tipo_lente.", ".$tipos_lentes_slct.",".$tipo_filtro_slct.",".$tiempo_vigencia_slct." ,".$tiempo_periodo.", ".$distancia_pupilar.", ".$form_cantidad.", ".$txt_observaciones_optometria.", @id)";
				//echo($sql);
		...
		public function getHcGafas($id_hc) {
			try {
				$sql = "SELECT C.* 
						...
						WHERE C.id_hc=".$id_hc." AND C.id_hc=HC.id_hc";		
