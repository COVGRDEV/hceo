<?php
	...
	class DbConvenios extends DbConexion {
		...
		public function guardarModificaConvenio($nombre, $indActivo, $idUsuarioCrea, $idAdmision, $tipoAccion, $indEco,
				$txtCodAdministradora, $indNumAut, $indNumAutObl, $indNumCarnet, $indNumCarnetObl, $idTipoDocumento, 
				$numeroDocumento,$fec_ini_fac,$fec_fin_fac,$contrato, $num_contrato) {
			try {
				if ($idTipoDocumento == "") {
				...
				$fec_ini_fac = $fec_ini_fac == "" ? "NULL" : $fec_ini_fac;
				$fec_fin_fac = $fec_fin_fac == "" ? "NULL" : $fec_fin_fac;
				$contrato = $contrato == "" ? "NULL" : $contrato;
				$num_contrato = $num_contrato == "" ? "NULL" : "'".$num_contrato."'";
				...
				$sql = "CALL pa_crear_modificar_convenio('".$nombre."', ".$indActivo.", ".$idUsuarioCrea.", ".$idAdmision.", ".$tipoAccion.", ".
						...
						$idTipoDocumento.", ".$numeroDocumento.", ".$fec_ini_fac.", ".$fec_fin_fac.", ".$contrato.", ".$num_contrato.", @id)";
		...
		public function getConvenio($id_convenio) {
			try {
				$sql = "SELECT 
						  C.*,
						  TD.codigo_detalle AS cod_tipo_documento,
						  TD.nombre_detalle AS tipo_documento,
						  TD.text_adicional_detalle AS codigo_doc_siesa,
						  CT.nombre_detalle AS contratacion_nombre
						FROM
						  convenios C 
						  LEFT JOIN listas_detalle TD 
							ON C.id_tipo_documento = TD.id_detalle 
						  LEFT JOIN listas_detalle CT 
							ON C.contratacion = CT.id_detalle 
		...
		public function crearModificarPlan($txtNombrePlan, $tipoPago, $indActivoPlan, $idConvenio, $idUsuarioCrea, $tipoAccion, $idPlan, $tipoUsuario,
				$codTipoMedicamento = "", $indISS2001 = "0", $iss2001porc = "", $idLiqQx = "", $indCalcCC = "0", $indDescCC = "1",$cmb_cobertura="") {
			try {
				...
				$sql = "CALL pa_crear_planes('".$txtNombrePlan."', ".$tipoPago.", ".$indActivoPlan.", ".$idConvenio.", ".$idUsuarioCrea.", ".$tipoAccion.", ".
						$idPlan.", ".$tipoUsuario.", ".$codTipoMedicamento.", ".$indISS2001.", ".$iss2001porc.", ".$idLiqQx.", ".$indCalcCC.", ".$indDescCC.", ".$cmb_cobertura.", @id)";
