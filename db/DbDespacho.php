<?php
	...
	class DbDespacho extends DbHistoriaClinica {
		...
		public function getRegistrosDespacho($id_paciente) {
	        try {
	            $sql = "SELECT p.*, d.*, c.nombre_tipo_cita, DATE_FORMAT(d.fecha_despacho, '%d/%m/%Y') AS fecha_despacho_t FROM despacho d
		...
		public function getListaDespachoCotizaciones($id_admision) {
	        try {
	            $sql = "SELECT DC.*, PC.nombre_proc_cotiz
		 ...
		 */
		public function getListaDespachoCotizacionesParams($txt_paciente, $id_proc_cotiz, $observaciones_cotiz,
				$fecha_ini, $fecha_fin, $id_usuario_prof = "", $id_lugar_cita = "") {
			try {
		...
		public function getRegistrosDespachoDet($id_paciente, $id_admision){
			...
			try {
				$sql = "SELECT p.*, d.*, c.nombre_tipo_cita, a.id_lugar_cita, a.motivo_consulta, a.fecha_admision, a.id_plan, a.id_convenio,
						DATE_FORMAT(d.fecha_despacho, '%d/%m/%Y') AS fecha_despacho_t 
						FROM despacho d
						INNER JOIN pacientes p ON p.id_paciente=d.id_paciente
						INNER JOIN admisiones a ON a.id_admision=d.id_admision
						INNER JOIN tipos_citas c ON c.id_tipo_cita=a.id_tipo_cita
	...
	public function GuardarIncapacidad($id_paciente,$id_profesional,$id_usuario,$tipo_atencion,$origen_incapacidad,$prorroga,$fecha_inicial,$fecha_final,
			$observaciones, $id_convenio,$id_plan,$id_lugar_cita, $id_hc,$id_admision,$diagnostico_principal,$diagnostico_relacionado){
				...
				$sql = "CALL pa_crear_incapacidad(".$id_paciente.", ".$id_profesional.", ".$id_usuario.", ".$tipo_atencion.", ".$origen_incapacidad.",
				".$prorroga.", '".$fecha_inicial."', '".$fecha_final."', '".$observaciones."', ".$id_convenio.", ".$id_plan.", ".$id_lugar_cita.", ".$id_hc.", 
				".$id_admision.",".$id_incapacidad.", '".$diagnostico_principal."', '".$diagnostico_relacionado."', @id)";
