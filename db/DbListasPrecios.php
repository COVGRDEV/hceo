<?php
	...
	class DbListasPrecios extends DbConexion {
		...
		public function getListasPreciosServicio($idPlan, $tipoServicio, $accion, $parametro) {
			try {
				...
				if ($tipoServicio == "P") {
					...
					$solicitud_aux = "LP.cod_insumo_siesa";
					$join_aux = " LEFT JOIN maestro_insumos MS ON MS.cod_insumo_siesa=LP.cod_insumo_siesa
								  INNER JOIN (
										SELECT L2.cod_insumo_siesa, L2.tipo_bilateral, MAX(L2.fecha_ini) AS fecha_ini
										FROM listas_precios L2
										WHERE L2.id_plan=".$idPlan."
										GROUP BY L2.cod_insumo_siesa, L2.tipo_bilateral
								  ) T ON LP.cod_insumo_siesa=T.cod_insumo_siesa AND LP.tipo_bilateral=T.tipo_bilateral AND LP.fecha_ini=T.fecha_ini ";
					$campo_aux = "MS.nombre_insumo AS nombre_aux, 1 AS ind_activo_aux, MS.cod_insumo_siesa";
					
					if ($parametro != "") {
						$buscaPrecio = "AND (MS.nombre_insumo LIKE '%".$parametro."%'
										OR MS.cod_insumo='".$parametro."' OR MS.cod_insumo_siesa='".$parametro."') ";
					}
		...
		public function guardarEditarListasPrecios($accion, $tipoServicio, $fechaInicial, $fechaFinal,
				$codServicio, $plan, $tipoBilateral, $valorTotal, $valorCuota, $idUsuarioCrea, $idPrecio,$codigoServicioSiesa="") {
			try {
				if ($fechaFinal == "") {
					$fechaFinal = "NULL";
				} else {
				...
				if ($codigoServicioSiesa == "") {
					$codigoServicioSiesa = "NULL";
				} else {
					$codigoServicioSiesa = "'".$codigoServicioSiesa."'";
				}
				
				$sql = "CALL pa_crear_editar_listas_precios(".$accion.", '".$tipoServicio."', '".$fechaInicial."', ".$fechaFinal.", '".
						$codServicio."', ".$plan.", ".$tipoBilateral.", ".$valorTotal.", ".$valorCuota.", ".$idUsuarioCrea.", ".$idPrecio.", ".$codigoServicioSiesa.", @id)";
				echo($sql);
				$arrCampos[0] = "@id";
		...
		public function getPrecio($idPrecio) {
			try {
				$sql = "SELECT LP.*, DATE_FORMAT(LP.fecha_ini, '%d/%m/%Y') AS fecha_ini_aux,
						...
						MM.nombre_generico, MI.nombre_insumo, MI.cod_insumo_siesa
						FROM listas_precios LP
		...
		public function getListasPreciosPorPlanParametro($id_plan, $parametro, $fecha = "", $ind_activo = "") {
			try {
				...
				$sql = "SELECT LP.*, MP.nombre_procedimiento, CONCAT(MM.nombre_comercial, ' - ', MM.nombre_generico) AS nombre_medicamento_aux, MI.nombre_insumo, P.ind_tipo_pago,
						MI.cod_insumo_siesa 
						FROM listas_precios LP
				...
				$sql .= "LEFT JOIN maestro_medicamentos MM ON MM.cod_medicamento=LP.cod_medicamento
						LEFT JOIN maestro_insumos MI ON MI.cod_insumo_siesa=LP.cod_insumo_siesa
				...
				if ($parametro != "") {
					$sql .= "AND (MP.nombre_procedimiento LIKE '%".$parametro."%'
							...
							OR MI.cod_insumo_siesa LIKE '%".$parametro."%') ";
				}
		...
		public function getHistorialPrecios($cod_procedimiento, $id_plan, $tipoServicio, $tipo_bilateral) {
			try {
				...
				if ($tipoServicio == "P") {
					...
					$solicitud_aux = "LP.cod_insumo_siesa";
					$join_aux = " LEFT JOIN maestro_insumos MI ON MI.cod_insumo_siesa=LP.cod_insumo_siesa";
		...
		public function getPrecioFecha($id_plan, $codigo, $tipo_precio, $tipo_bilateral, $fecha = "") {
			try {
				...
				$sql = "SELECT LP.*, MP.nombre_procedimiento, CONCAT(MM.nombre_comercial, ' - ', MM.nombre_generico) AS nombre_medicamento_aux, MI.nombre_insumo
						FROM listas_precios LP
						LEFT JOIN maestro_procedimientos MP ON MP.cod_procedimiento=LP.cod_procedimiento
						LEFT JOIN maestro_medicamentos MM ON MM.cod_medicamento=LP.cod_medicamento
						LEFT JOIN maestro_insumos MI ON MI.cod_insumo=LP.cod_insumo
