<?php
	...
	class DbPagos extends DbConexion {
			...
			try {
				if ($accion == "0") {
					...
					$sql = "SELECT P.*, PC.nombre_1, PC.nombre_2, PC.apellido_1, PC.apellido_2, PC.id_tipo_documento, PC.numero_documento,
							PC.direccion, PC.telefono_1, PC.telefono_2, PC.email, PC.tipo_coti_paciente, LD.nombre_detalle AS tipo_documento, LD.codigo_detalle AS cod_tipo_documento,
							M.nom_mun AS nom_mun_t, D.nom_dep AS nom_dep_t, PC.nom_dep, PC.nom_mun, PA.nombre_pais, M.cod_mun_dane,
							DATE_FORMAT(P.fecha_pago, '%d/%m/%Y') AS fecha_pago_t, TC.id_tipo_cita, TC.nombre_tipo_cita, U.nombre_usuario, U.apellido_usuario,
		...
		$tipoPago2, $BancoPago2, $valorPago2, $tipoPago3, $BancoPago3, $valorPago3, $idPlan, $idConvenio, $num_factura, $cmbEntidad, 
		$observacionesPago, $estado_pago, $arr_pagos_detalle = array(), $arr_medios_pago = array(), $idLugarCita = "", $idUsuarioProf = "", 
		$idUsuarioProfFormula="", $nombreProfFormula="", $id_tercero = "", $num_pedido = "",$num_mipress = "", $num_ent_mipress = "", $num_poliza="") {
			try {
				...
				for ($i = 0; $i < count($arr_pagos_detalle); $i++) {
					...
					$resultado_aux = $this->crearTemporalPagosDetallePagos2($idAdmision, $detalle_aux["id_detalle_precio"], $detalle_aux["cod_servicio"],
							...
							$detalle_aux["valor_cuota"], $detalle_aux["tipo_precio"], $idUsuarioCrea, $semilla,$detalle_aux["cod_servicio_siesa"]);
				...
				if ($resultado_aux > 0 && count($arr_pagos_detalle) > 0) {
					...
					if ($idUsuarioProfFormula == "") {
						$idUsuarioProfFormula = $idUsuarioProf;
					}
					...
					if (trim($nombreProfFormula) == "") {
						$nombreProfFormula = "NULL";
					} else {
						$nombreProfFormula = "'".$nombreProfFormula."'";
					}
					
					
					$sql = "CALL pa_pagos(".$idPago.", ".$idAdmision.", ".$idPaciente.", ".$idLugarCita.", ".$idUsuarioProf.", ".$idUsuarioProfFormula.",  
					".$nombreProfFormula.",					
					".$idConvenio.", ".$idPlan.", ".$num_factura.", ".$cmbEntidad.", ".$observacionesPago.", ".$estado_pago.", 
					".$id_tercero.", ".$num_pedido.", ".$num_mipress.", ".$num_ent_mipress.",".$num_poliza.", ".$idUsuarioCrea.", ".$semilla.", @id)";
		...
		public function reporteTesoseriaProcedimientosPlanes($fechaInicial, $fechaFinal, $id_convenio, $id_plan, $id_lugar_cita, $cod_insumo, $tipo_precio, $idUsuario, $idUsuarioAdm = "") {
			try {
				...
				if ($tipo_precio == "" || $tipo_precio == "I") {
					...
					$sql .= "SELECT PD.cod_insumo_siesa AS cod_insumo, MI.nombre_insumo, PD.tipo_precio
							FROM pagos_detalle PD
							INNER JOIN pagos P ON PD.id_pago=P.id_pago
					...
					if ($cod_insumo != "") {
						$sql .= "AND MI.cod_insumo_siesa LIKE '%".$cod_insumo."%'";
					}
					if ($idUsuarioAdm != '') {
						$sql .= "AND IFNULL(A.id_usuario_crea, P.id_usuario_crea)=".$idUsuarioAdm." ";
					}
		...
		public function reporteTesoseriaProcedimientosPacientesPlanes($fechaInicial, $fechaFinal, $id_convenio, $id_plan, $id_lugar_cita, $cod_insumo, $tipo_pago, $idUsuario, $idUsuarioAdm = "") {
			try {
				...
				if ($tipo_pago == "" || $tipo_pago == "I") {
					...
					$sql .= "SELECT PD.cod_insumo_siesa AS cod_insumo,  MI.nombre_insumo, PD.id_pago, PD.tipo_precio, PD.id_admision,
		...
		public function getListaReporteTesoseriaAuditoria($fecha_ini, $fecha_fin, $id_convenio, $estado_pago) {
			try {
				...
				$sql .=
							"UNION ALL
							SELECT P.id_pago, PD.cod_insumo_siesa AS cod_insumo, MI.nombre_insumo,
							PD.tipo_precio, PD.valor, PD.valor_cuota, PD.cantidad, LP.valor AS valor_base, LP.valor_cuota AS valor_cuota_base
							FROM pagos P
							INNER JOIN pagos_detalle PD ON P.id_pago=PD.id_pago
							INNER JOIN maestro_insumos MI ON PD.cod_insumo_siesa=MI.cod_insumo_siesa
		...
		*/
			...
			try {
				$sql = "SELECT PA.id_pago, PDA.cod_insumo, PDA.nombre_insumo, PAS.nombre_1, PAS.nombre_2, PAS.apellido_1, PAS.apellido_2,
						...
						FROM (
							...
							SELECT PD.cod_insumo_siesa AS cod_insumo, MI.nombre_insumo, PD.id_pago, PD.tipo_precio, PD.id_admision,
							PD.valor, PD.valor_cuota, PD.cantidad, LP.valor AS valor_b, LP.valor_cuota AS valor_cuota_b
							FROM pagos_detalle PD
							INNER JOIN pagos PA ON PD.id_pago=PA.id_pago
							INNER JOIN maestro_insumos MI ON PD.cod_insumo_siesa=MI.cod_insumo_siesa
							LEFT JOIN admisiones A ON PA.id_admision=A.id_admision
							LEFT JOIN listas_precios LP ON PA.id_plan=LP.id_plan AND PD.cod_insumo_siesa=LP.cod_insumo_siesa
							AND PD.tipo_bilateral=LP.tipo_bilateral
							AND LP.fecha_ini<=DATE(PA.fecha_pago) AND IFNULL(LP.fecha_fin, DATE(PA.fecha_pago))>=DATE(PA.fecha_pago)
							WHERE DATE(PA.fecha_pago) BETWEEN '".$fechaInicial."' AND '".$fechaFinal."'
							AND PA.estado_pago=2
		...
		public function crearTemporalPagosDetallePagos2($id_admision, $id_detalle_precio, $cod_servicio, $tipo_bilateral,
				$num_autorizacion, $cantidad, $valor, $valor_cuota, $tipo_precio, $id_usuario, $semilla, $cod_servicio_siesa="") {
			try {
				if ($id_admision == "") {
					$id_admision = "NULL";
				}
				...
				if ($cod_servicio_siesa == "") {
					$cod_servicio_siesa = "NULL";
				}else{
					$cod_servicio_siesa = "'".$cod_servicio_siesa."'";
				}
				
				$sql = "CALL pa_crear_editar_pagos_detalle(".$id_admision.", ".$id_detalle_precio.", '".$cod_servicio."', ".$cod_servicio_siesa.",
				".$tipo_bilateral.", ".$num_autorizacion.", ".$cantidad.", ".$valor.", ".$valor_cuota.", '".$tipo_precio."', 
				".$id_usuario.", ".$semilla.", @id)";
				//echo($sql);
			...
			try {
				...
				if ($tipo_precio == "" || $tipo_precio == "P") {
					...
					$sql = "SELECT P.id_pago, P.fecha_pago, CO.nombre_convenio, PL.nombre_plan, LD.nombre_detalle AS sede,
							...
							CONCAT(UF.nombre_usuario, ' ', UF.apellido_usuario) AS profesional_formula,
							...
							LEFT JOIN usuarios UP ON P.id_usuario_pago=UP.id_usuario
							LEFT JOIN usuarios UF ON P.id_usuario_prof_formula=UF.id_usuario
							WHERE P.fecha_pago BETWEEN '".$fechaInicial."' AND '".$fechaFinal." 23:59:59'
							AND P.estado_pago=2 ";
					if ($id_convenio != "") {
						$sql .= "AND P.id_convenio=".$id_convenio." ";
				...
				if ($tipo_precio == "" || $tipo_precio == "I") {
					...
					$sql .= "SELECT P.id_pago, P.fecha_pago, CO.nombre_convenio, PL.nombre_plan, LD.nombre_detalle AS sede,
							CONCAT(UP.nombre_usuario, ' ', UP.apellido_usuario) AS usuario_registra_pago,
							CONCAT(U.nombre_usuario, ' ', U.apellido_usuario) AS medico, PD.cod_insumo_siesa AS cod_insumo, MI.nombre_insumo, PD.tipo_precio, PD.tipo_bilateral,
							PC.nombre_1, PC.nombre_2, PC.apellido_1, PC.apellido_2,
							CONCAT(PC.nombre_1, ' ', IFNULL(PC.nombre_2, ''), ' ', PC.apellido_1, ' ', IFNULL(PC.apellido_2, '')) AS nombre_completo,
							CONCAT(UF.nombre_usuario, ' ', UF.apellido_usuario) AS profesional_formula,
							TD.nombre_detalle AS tipo_documento, PC.numero_documento, PC.ind_habeas_data, PD.cantidad, PD.valor, PD.valor_cuota,
							...
							FROM pagos_detalle PD
							INNER JOIN pagos P ON PD.id_pago=P.id_pago
							INNER JOIN maestro_insumos MI ON MI.cod_insumo_siesa=PD.cod_insumo_siesa
							INNER JOIN pacientes PC ON PC.id_paciente=P.id_paciente
							LEFT JOIN listas_detalle TD ON PC.id_tipo_documento=TD.id_detalle
							LEFT JOIN terceros T ON P.id_tercero=T.id_tercero
							LEFT JOIN listas_detalle DT ON T.id_tipo_documento=DT.id_detalle
		...
		public function get_reporte_coomeva_excel($fecha_inicial, $fecha_final, $id_convenio){
			try {
				$sql = "SELECT DISTINCT DH.cod_ciex, PD.num_autorizacion, PD.cod_procedimiento AS codigo_diagnostico,  
						CONCAT(PC.nombre_1, ' ',PC.apellido_1) AS paciente, PC.numero_documento AS identificacion, PD.cod_procedimiento AS CUPS,
						DATE(P.fecha_pago) AS fecha_autorizacion, PD.valor AS tarifa_negociada, PD.valor_cuota AS cupon_pago_directo,
						(PD.valor-PD.valor_cuota) AS valor_a_pagar
						FROM pagos P
						INNER JOIN pacientes PC ON PC.id_paciente = P.id_paciente
						LEFT JOIN pagos_detalle PD ON PD.id_pago = P.id_pago
						INNER JOIN historia_clinica HC ON HC.id_paciente = PC.id_paciente
						INNER JOIN diagnosticos_hc DH ON HC.id_hc = DH.id_hc
						WHERE (P.fecha_pago BETWEEN STR_TO_DATE('".$fecha_inicial."', '%d/%m/%Y') AND STR_TO_DATE('".$fecha_final." 23:59:59', '%d/%m/%Y %H:%i:%s')) 
						AND (HC.fecha_crea BETWEEN STR_TO_DATE('".$fecha_inicial."', '%d/%m/%Y') AND STR_TO_DATE('".$fecha_final." 23:59:59', '%d/%m/%Y %H:%i:%s'))						
						AND P.id_convenio =".$id_convenio."
						GROUP BY PD.id_detalle_precio;";
		...
		public function getPagosReporteProductividad($fecha_inicial, $fecha_final, $id_sede, $tipo_consulta){
			
			try {
				$filtro=""; $agrupado=""; $encabezado=""; $join="";
				
				if($id_sede <> ""){
					$filtro = " AND P.id_lugar_cita = ".$id_sede; 
					
				}
				
				if($tipo_consulta==0){
					
					$encabezado = " CONCAT(PA.nombre_usuario,' ',PA.apellido_usuario) AS profesional, ";
					$join=" INNER JOIN usuarios PA ON PA.id_usuario = P.id_usuario_prof ";
					$agrupado=" GROUP BY P.id_usuario_prof, ";
					
				}else if($tipo_consulta==1){
					
					$encabezado = "  CONCAT(PF.nombre_usuario,' ', PF.apellido_usuario) AS profesional, ";
					$join=" INNER JOIN usuarios PF ON PF.id_usuario = P.id_usuario_prof_formula  ";
					$agrupado="  GROUP BY P.id_usuario_prof_formula, ";
				...
				if($tipo_consulta==2){
					...
					$join=" INNER JOIN usuarios PF ON PF.id_usuario = P.id_usuario_prof_formula  ";
					$agrupado="  GROUP BY P.id_usuario_prof_formula, ";
					$filtro.=" AND P.id_usuario_prof <> P.id_usuario_prof_formula";
	
				}
				
				$sql= " SELECT $encabezado
					  S.nombre_detalle AS sede,
					  SUM(PD.valor) AS producido,
					  COUNT(*) AS cantidad
					  FROM pagos P
					  INNER JOIN pagos_detalle PD ON PD.id_pago = P.id_pago 
					  $join
					  INNER JOIN listas_detalle S ON S.id_detalle = P.id_lugar_cita 
					  WHERE P.fecha_pago BETWEEN '$fecha_inicial 00:00:00' 
					  AND '$fecha_final 23:59:59' 
					  AND P.estado_pago=2 $filtro
					  $agrupado P.id_lugar_cita
