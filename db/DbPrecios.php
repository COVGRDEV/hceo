...
class DbPrecios extends DbConexion {
        ...
        try {
			if ($id_admision_paciente > 0) {
				$sql = "SELECT '99' AS orden, MP.cod_procedimiento, MP.nombre_procedimiento, pd.valor, 
						...
						LP.cod_insumo, LP.cod_insumo_siesa, p.ind_tipo_pago, pd.tipo_precio, pd.tipo_bilateral, 0 AS id_examen,
				...
				$sql = "SELECT TCD.orden, MP.cod_procedimiento, MP.nombre_procedimiento, LP.valor, LP.valor_cuota, LP.id_precio,
						LP.cod_procedimiento, LP.cod_medicamento, LP.cod_insumo, LP.cod_insumo_siesa, P.ind_tipo_pago, LP.tipo_precio, LP.tipo_bilateral,
    ...
    public function getTodosLosPrecios($id_plan, $parametro, $ind_activo = "") {
        try {
			...
			$sql .= "AND (LP.cod_procedimiento LIKE '%".$parametro."%'
					...
					FROM listas_precios LP
					INNER JOIN maestro_insumos MI ON LP.cod_insumo_siesa=MI.cod_insumo_siesa
					INNER JOIN planes P ON LP.id_plan=P.id_plan
					WHERE LP.id_plan=".$id_plan."
					AND LP.fecha_ini<=DATE(NOW())
					AND IFNULL(LP.fecha_fin, DATE(NOW()))>=DATE(NOW())
					AND (LP.cod_insumo_siesa LIKE '%".$parametro."%'
    ...
    public function getListaPreciosOtrosPlanes($id_plan, $parametro, $ind_activo = "") {
        try {
            $parametro = str_replace(" ", "%", $parametro);   
            $sql = "SELECT DISTINCT NULL AS id_precio, P.id_plan, MP.cod_procedimiento, NULL AS cod_medicamento,NULL AS cod_insumo , NULL AS cod_insumo_siesa,
			...
			$sql .= "AND (MP.cod_procedimiento LIKE '%".$parametro."%'
					...
					SELECT DISTINCT NULL AS id_precio, P.id_plan, NULL, MM.cod_medicamento, NULL ,NULL, NULL, NULL,
					...
					SELECT DISTINCT NULL AS id_precio, P.id_plan, NULL, NULL, MI.cod_insumo ,MI.cod_insumo_siesa, NULL, NULL,
					0 AS valor, 0 AS valor_cuota, 'I' AS tipo_precio,
					TB.tipo_bilateral, MI.nombre_insumo, P.ind_tipo_pago
					FROM maestro_insumos MI
					INNER JOIN listas_precios LP ON MI.cod_insumo_siesa=LP.cod_insumo_siesa
					CROSS JOIN planes P
					CROSS JOIN (
					...
					WHERE LP.fecha_ini<=DATE(NOW())
					AND IFNULL(LP.fecha_fin, DATE(NOW()))>=DATE(NOW())
					AND (MI.cod_insumo_siesa LIKE '%".$parametro."%'
					OR MI.nombre_insumo LIKE '%".$parametro."%')
					AND P.id_plan=".$id_plan."
    ...
    public function getPreciosAdmision($id_admision) {
        try {
			$sql = "SELECT P.id_pago, P.id_admision, '99' AS orden, MP.cod_procedimiento, MP.nombre_procedimiento, PD.valor, PD.valor_cuota,
					LP.id_precio, MP.cod_procedimiento, LP.cod_medicamento, LP.cod_insumo, LP.cod_insumo_siesa, P.ind_tipo_pago, PD.tipo_precio, PD.tipo_bilateral,
					...
					LP.id_precio, LP.cod_procedimiento, MM.cod_medicamento, LP.cod_insumo, LP.cod_insumo_siesa,  P.ind_tipo_pago, PD.tipo_precio, PD.tipo_bilateral,
					0 AS id_examen, PD.cantidad, P.id_convenio, P.id_plan, PD.num_autorizacion
					FROM pagos P
					INNER JOIN pagos_detalle PD ON P.id_pago=PD.id_pago
					...
					LP.id_precio, LP.cod_procedimiento, LP.cod_medicamento, MI.cod_insumo, MI.cod_insumo_siesa, P.ind_tipo_pago, PD.tipo_precio, PD.tipo_bilateral,
					0 AS id_examen, PD.cantidad, P.id_convenio, P.id_plan, PD.num_autorizacion
					FROM pagos P
					INNER JOIN pagos_detalle PD ON P.id_pago=PD.id_pago
					INNER JOIN maestro_insumos MI ON PD.cod_insumo_siesa=MI.cod_insumo_siesa
					LEFT JOIN listas_precios LP ON PD.cod_insumo_siesa=LP.cod_insumo_siesa AND PD.tipo_bilateral=LP.tipo_bilateral AND P.id_plan=LP.id_plan
					AND LP.fecha_ini<=CURDATE() AND IFNULL(LP.fecha_fin, CURDATE())>=CURDATE()
					WHERE P.id_admision=".$id_admision."
					AND P.estado_pago=1
		...
		try{
			switch ($tipo_servicio) {
				case "P": //PROCEDIMIENTOS
						$sql = "SELECT 
							  MP.cod_procedimiento AS cod_servicio, LP.fecha_ini, LP.fecha_fin, 
							  LP.valor_cuota, LP.tipo_bilateral, LP.ind_liquidacion, LP.tipo_liquidacion,
							  NULL AS cod_item_siesa, LP.valor
							FROM
							  listas_precios LP 
							  LEFT JOIN maestro_procedimientos MP 
								ON MP.cod_procedimiento = LP.cod_procedimiento 
							WHERE LP.id_plan = ".$id_plan." 
							  AND LP.tipo_precio = 'P' 
							  AND LP.fecha_fin IS NULL 
							ORDER BY LP.cod_insumo";		
					break;
				case "I"://INSUMOS 
					$sql = "SELECT 
							  LP.cod_insumo AS cod_servicio,
							  LP.fecha_ini,
							  LP.fecha_fin,
							  LP.valor_cuota,
							  LP.tipo_bilateral,
							  LP.ind_liquidacion,
							  LP.tipo_liquidacion,
							  LP.cod_insumo_siesa AS cod_item_siesa,
							  LP.valor 
							FROM
							  listas_precios LP 
							  LEFT JOIN maestro_insumos MI 
								ON MI.cod_insumo_siesa = LP.cod_insumo_siesa 
							WHERE LP.id_plan = ".$id_plan."
							  AND LP.tipo_precio = 'I' 
							  AND LP.fecha_fin IS NULL 
							ORDER BY LP.cod_insumo_siesa DESC "; 
