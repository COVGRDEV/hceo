<?php
	...
	class DbRIPS extends DbConexion {
		...
		public function crear_rips_usuarios($id_rips_us, $id_rips, $id_paciente, $ind_revisado, $ind_borrado, $tipo_documento, $numero_documento, $nombre_1, $nombre_2,
				$apellido_1, $apellido_2, $cod_administradora, $tipo_usuario, $edad, $unidad_edad, $sexo, $cod_dep, $cod_mun, $zona, $observaciones, $id_usuario) {
			try {
				if ($id_paciente == "") {
					$id_paciente = "NULL";
				}
				...
				$sql = "CALL pa_crear_rips_usuarios(".$id_rips_us.", ".$id_rips.", ".$id_paciente.", ".$ind_revisado.", ".$ind_borrado.", ".$tipo_documento.", ".
						...
						$edad.", ".$unidad_edad.", ".$sexo.", ".$cod_dep.", ".$cod_mun.", ".$zona.", ".$observaciones.", ".$id_usuario.", @id)";
				//echo($sql);
		...
		public function crear_rips_directo($id_rips, $id_prestador, $id_usuario, $num_factura) {
			try {
				$sql = "CALL pa_crear_rips_directo(".$id_rips.", ".$id_prestador.", ".$id_usuario.", '$num_factura', @id)";
				//echo($sql);
		...
		public function get_lista_registros_ac($id_convenio, $id_plan, $fecha_inicial, $fecha_final, $ind_rips_hc, $ind_sin_np = 0) {
			try {
				$sql = "/*Registros con historia clínica*/
						...
						P.id_paciente, NULL AS num_factura, DATE_FORMAT(IFNULL(HC.fecha_hora_hc, P.fecha_pago), '%d/%m/%Y') AS fecha_consulta_t,
				...
				$sql .= "UNION ALL
						...
						P.id_paciente, NULL AS num_factura, DATE_FORMAT(P.fecha_pago, '%d/%m/%Y') AS fecha_consulta_t,
						IFNULL(PD.num_autorizacion, A.num_autorizacion) AS num_autorizacion, PD.cod_procedimiento, MP.nombre_procedimiento, NULL AS fin_consulta,
						NULL AS causa_ext, NULL AS cod_ciex_prin, NULL AS cod_ciex_rel1, NULL AS cod_ciex_rel2,
						NULL AS cod_ciex_rel3, NULL AS tipo_diag_prin, PD.tipo_precio, PD.cantidad, 0 AS valor_consulta,
						0 AS valor_cuota, 0 AS valor_neto, PD.valor, PD.valor AS valor_p, PD.valor_cuota AS valor_cuota_p,
				...
				if ($ind_rips_hc == 1) {
					...
					$sql .= 	"AND PD.id_detalle_precio=RC.id_detalle_precio
							...
							RC.id_paciente, RC.num_factura, DATE_FORMAT(RC.fecha_consulta, '%d/%m/%Y') AS fecha_consulta_t,
		...
		public function get_lista_rips_consultas($id_convenio, $id_plan, $fecha_ini, $fecha_fin, $ind_borrado) {
			try {
				$sql = "SELECT RC.*, MP.nombre_procedimiento, P.nombre_1, P.nombre_2, P.apellido_1, P.apellido_2,
						...
						D3.nombre AS nom_ciex_rel3, DATE_FORMAT(RC.fecha_consulta, '%d/%m/%Y') AS fecha_consulta_t, 1 AS cantidad
						FROM rips_consultas RC
						LEFT JOIN maestro_procedimientos MP ON RC.cod_procedimiento=MP.cod_procedimiento
						LEFT JOIN pacientes P ON RC.id_paciente=P.id_paciente
						LEFT JOIN ciex DP ON RC.cod_ciex_prin=DP.codciex
		...
		public function get_lista_registros_am($id_convenio, $id_plan, $fecha_inicial, $fecha_final, $ind_rips_hc, $ind_sin_np = 0) {
			try {
				$sql = "/*Registros de historia clínica*/
						SELECT 0 AS id_rips_am, PD.id_detalle_precio, P.id_admision, P.id_convenio, P.id_plan, P.id_paciente,
						NULL AS num_factura, DATE_FORMAT(A.fecha_admision, '%d/%m/%Y') AS fecha_medicamento_t, IFNULL(PD.num_autorizacion, A.num_autorizacion) AS num_autorizacion,
						PD.cod_medicamento, MM.nombre_generico, CASE MM.ind_pos WHEN 1 THEN '1' ELSE '2' END AS tipo_medicamento,
						MM.presentacion AS forma_farma, MM.concentracion, MM.unidad_medida, PD.tipo_precio, PD.cantidad, 0 AS valor_medicamento,
				...
				if ($ind_rips_hc == 1) {
					...
					$sql .= "AND NOT EXISTS (
								SELECT RM.id_detalle_precio
								FROM rips_medicamentos RM
								WHERE RM.id_convenio=".$id_convenio." ";
					if ($id_plan > 0) {
						$sql .= "AND RM.id_plan=".$id_plan." ";
					}
					$sql .= 	"AND RM.id_detalle_precio=PD.id_detalle_precio
							)
							
							UNION ALL
							
							/*Registros de RIPS*/
							SELECT RM.id_rips_am, RM.id_detalle_precio, RM.id_admision, RM.id_convenio, RM.id_plan, RM.id_paciente,
							RM.num_factura, DATE_FORMAT(RM.fecha_medicamento, '%d/%m/%Y') AS fecha_medicamento_t, RM.num_autorizacion,
							RM.cod_medicamento, RM.nombre_generico, RM.tipo_medicamento, RM.forma_farma, RM.concentracion,
							RM.unidad_medida, 0 AS tipo_precio, RM.cantidad, RM.valor_medicamento, 0 AS valor, 0 AS valor_p,
							0 AS valor_cuota_p, RM.tipo_documento, RM.numero_documento, P.nombre_1, P.nombre_2, P.apellido_1,
							P.apellido_2, NULL AS ind_tipo_pago, RM.ind_revisado, RM.ind_borrado, RM.observaciones
							FROM rips_medicamentos RM
					...
					$sql .= "ORDER BY -id_admision DESC, id_detalle_precio, id_rips_am";
				}
				//echo($sql);
		...
		public function get_lista_registros_at($id_convenio, $id_plan, $fecha_inicial, $fecha_final, $ind_rips_hc, $ind_sin_np = 0) {
			try {
				$sql = "/*Registros de historia clínica*/
						...
						NULL AS num_factura, DATE_FORMAT(A.fecha_admision, '%d/%m/%Y') AS fecha_insumo_t, IFNULL(PD.num_autorizacion, A.num_autorizacion) AS num_autorizacion,
						TI.codigo_detalle AS tipo_insumo, PD.cod_insumo_siesa AS cod_insumo, MI.nombre_insumo, PD.tipo_precio, PD.cantidad, 0 AS valor_insumo,
						PD.valor, PD.valor AS valor_p, PD.valor_cuota AS valor_cuota_p, TD.codigo_detalle AS tipo_documento, PA.numero_documento,
						PA.nombre_1, PA.nombre_2, PA.apellido_1, PA.apellido_2, PL.ind_tipo_pago, 0 AS ind_revisado, 0 AS ind_borrado, NULL AS observaciones
						FROM pagos P
						INNER JOIN pagos_detalle PD ON P.id_pago=PD.id_pago
		...
		public function get_lista_registros_us($id_convenio, $id_plan, $fecha_inicial, $fecha_final, $ind_rips_hc, $ind_sin_np = 0) {
			try {
				$sql = "SELECT DISTINCT 0 AS id_rips_us, 0 AS id_rips, PA.id_paciente, TD.codigo_detalle AS tipo_documento, PA.numero_documento,
						...
						SX.codigo_detalle AS sexo, PA.cod_dep, M.cod_mun, ZN.codigo_detalle AS zona, IFNULL(DS.codigo_detalle, '2') AS desplazado,
