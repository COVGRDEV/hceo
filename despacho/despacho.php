    ...
    ?>
            ...
            <div class="tabs-container">
				<dl class="tabs" data-tab>
                	<dd class="active"><a href="#panel2-1" onclick="setTimeout(function() { ajustar_textareas(); }, 100);">F&Oacute;RMULAS M&Eacute;DICAS</a></dd>
                    <?php
if ($formula_gafas == 1) {
        ?>
                    <dd><a href="#panel2-2" onclick="setTimeout(function() { ajustar_textareas(); }, 100);">F&Oacute;RMULA DE GAFAS</a></dd>
                    <?php
}
    ?>
                    <dd><a href="#panel2-3" onclick="setTimeout(function() { ajustar_textareas(); }, 100);">COTIZACIONES</a></dd>
                    <?php
if ($ind_incapacidad == 1) {
        ?>
                    <dd><a href="#panel2-4" onclick="setTimeout(function() { ajustar_textareas(); }, 100);">INCAPACIDADES</a></dd>
                    <?php
}
    ?>
                </dl>
                                ...
                                <table border="0" cellpadding="5" cellspacing="0" align="center" style="width:100%;">
                                    <tr>
                                        ...
                                        <td align="left" style="width:30%;">
                                        	<input type="text" name="txt_remitido_<?php echo ($i); ?>" id="txt_remitido_<?php echo ($i); ?>" class="input input_hc" style="width:200px;" value="<?php echo ($remitido_aux); ?>" onblur="convertirAMayusculas(this); trim_cadena(this);" />
                                        </td>
...
$observaciones_cotiz_aux = $utilidades->ajustar_texto_wysiwyg($observaciones_cotiz_aux);
        ?>
                                        ...
                                        <!--<textarea id="txt_observaciones_cotiz_<?php echo ($i); ?>" class="textarea_oftalmo" style="height:120px; width:100%;"><?php echo ($observaciones_cotiz_aux); ?></textarea>-->
                                    </td>
                                </tr>
                                <?php
if ($ind_editar == 1) {
        ...
        ?>
					<div class="content" id="panel2-4">
							...
							<table border="0" cellpadding="5" cellspacing="0" align="center" style="width:95%;">
								<tr>
									<td align="center" colspan="4">
										<label><b>IMPRIMIR INCAPACIDAD</b></label>
							...
							<table id="tabla_refrafinal" class="modal_table" cellpadding="5" cellspacing="0" align="center" style="width:95%;">
								<tr>
								   <td class="td_reducido" align="right" style="width:10%;"><label class="inline">Tipo de atención:</label></td>
								   <?php
$incapacidades = $dbHistoriaClinica->getIncapacidades($id_admision);
        ...
        if (count($despacho_det) > 0) {
            ?>
                                                 	<input type="hidden" id="hdd_rta_hc" nombre="hdd_rta_hc" value="1"/>
                                                <?php
}
        ?>
                                    <input type="hidden" id="hdd_id_usuario" nombre="hdd_id_usuario_hc" value="<?=$_SESSION["idUsuario"];?>"/>
                                    <input type="hidden" id="hdd_id_paciente_hc" nombre="hdd_id_paciente_hc" value="<?=$despacho_det["id_paciente"];?>"/>
                                    <input type="hidden" id="hdd_id_convenio_hc" nombre="hdd_id_convenio_hc" value="<?=$despacho_det["id_convenio"];?>" />
                                    <input type="hidden" id="hdd_id_plan_hc" nombre="hdd_id_plan_hc" value="<?=$despacho_det["id_plan"];?>" />
                                    <input type="hidden" id="hdd_id_lugar_cita_hc" nombre="hdd_id_lugar_cita_hc" value="<?=$despacho_det["id_lugar_cita"];?>" />
                                    <input type="hidden" id="hdd_id_profesional_j" nombre="hdd_id_profesional_j" value="<?=$id_usuario_prof?>" />
                                    ...
                                    <input type="hidden" id="hdd_id_convenio" nombre="hdd_id_convenio"  value="<?=$despacho_det["id_convenio"];?>" />
									<input type="hidden" id="hdd_id_plan" nombre="hdd_id_plan"  value="<?=$despacho_det["id_plan"];?>" />
									<input type="hidden" id="hdd_id_lugar_cita" nombre="hdd_id_lugar_cita"  value="<?=$despacho_det["id_lugar_cita"];?>" />
									<input type="hidden" id="hdd_nombre_tipo_cita" nombre="hdd_nombre_tipo_cita"  value="<?=$despacho_det["nombre_tipo_cita"];?>" />
									<input type="hidden" id="hdd_id_admision_incapacidad" nombre="hdd_id_admision_incapacidad"  value="<?=$id_admision?>" />
                                    <input type="hidden" id="hdd_id_hc" nombre="hdd_id_hc"  value="<?=$id_hc?>" />

								   <td align="left" style="width:10%;">
									   <select class="select" style="width:100%;" id="cmb_tipo_atencion_0" name="cmb_tipo_atencion_0" onchange="" tabindex="">
										     <?php
if (!is_null($incapacidades["tipo_atencion"]) || !empty($incapacidades["tipo_atencion"])) {
            ?>
                                                 <option selected="selected" class="dos" value="<?=$incapacidades["tipo_atencion"]?>"><?=$incapacidades["nombre_atencion"]?></option>
											<?php
} else {

            ?>
                                                 <option class="dos" value="">--Seleccione--</option>
                                            <?php
}
        ...
        foreach ($tipos_atencion as $tipo) {
            ?>
										 <option class="dos" value="<?=$tipo["id_detalle"];?>"><?=$tipo["nombre_detalle"];?></option>
									<?php
}
        ?>
										</select>
								   </td>
									<td class="td_reducido" align="right" style="width:10%;"><label class="inline">Prórroga:</label></td>
									<td align="left" style="width:10%;">
										<select class="select" style="width:100%;" id="cmb_prorroga_0" name="cmb_prorroga_0" onchange="" tabindex="">
									 <?php
if (!is_null($incapacidades["prorroga"]) || !empty($incapacidades["prorroga"])) {
            if ($incapacidades["prorroga"] == 1) {
                ?>
												  <option class="uno" value="1">Sí</option>
												 <?php
} else if ($incapacidades["prorroga"] == 2) {
                ?>
													 <option class="uno" value="2">No</option>
												 <?php
}
        } else {
            ?>
        ...
        ?>
								   ...
								   <td class="td_reducido" align="right" style="width:10%;"><label class="inline">Origen de Incapacidad:</label></td>
									<td align="left" style="width:20%;">
									   <select class="select" style="width:100%;" id="cmb_origen_incapacidad_0" name="cmb_origen_incapacidad_0" onchange="" tabindex="">
										 <?php
										if (!is_null($incapacidades["origen_incapacidad"]) || !empty($incapacidades["origen_incapacidad"])) {
										?>
                                               <option selected="selected" class="dos" value="<?=$incapacidades["origen_incapacidad"]?>"><?=$incapacidades["nombre_origen_inc"]?></option>
										<?php
										} else {
										...
										foreach ($origenes_incapacidades as $value) {
											?>
												<option class="dos" value="<?=$value["id_detalle"];?>"><?=$value["nombre_detalle"];?></option>
											<?php
												}
											?>
									   </select>
								...
								<tr>
                                 	<?php
										if (!is_null($incapacidades["fecha_inicio_incapacidad"])) {
													$fecha_inicio = $incapacidades["fecha_inicio_incapacidad"];
													$fecha_fin = $incapacidades["fecha_fin_incapacidad"];
													$fecha_inicio = $dbVariables->cambiarFormatoFecha($fecha_inicio);
													$fecha_fin = $dbVariables->cambiarFormatoFecha($fecha_fin);
												} else {
													$fecha_inicio = "";
													$fecha_fin = "";
												}
									...
									<td>
										<input type="text" class="input" maxlength="10" style="width:120px;" name="fechaInicial" id="fechaInicial" onkeyup="DateFormat(this, this.value, event, false, '3');"  onchange="calcular_diff_fechas();" onfocus="vDateType = '3';"  onblur="DateFormat(this, this.value, event, true, '3'); calcular_diff_fechas();" value="<?=$fecha_inicio;?>">

									</td>
									<td class="td_reducido" align="right" style="width:10%;"><label class="inline">Fecha Fin:</label></td>
									<td>
										<input type="text" class="input" maxlength="10" style="width:120px;" name="fechaFinal" id="fechaFinal" onkeyup="DateFormat(this, this.value, event, false, '3');"  onchange="calcular_diff_fechas();" onfocus="vDateType = '3';"  onblur="DateFormat(this, this.value, event, true, '3'); calcular_diff_fechas();"  value="<?=$fecha_fin;?>">
								...
								<tr>
									  <td align="left" colspan="8">
											...
											<?php
												$observaciones = $incapacidades["observaciones"];
														$observaciones_adicionales_aux = $utilidades->ajustar_texto_wysiwyg($observaciones);

											?>
											<div id="txt_observaciones_adicionales_<?php echo (0); ?>"><?php echo (trim(strip_tags($observaciones))); ?></div>
									   </td>
								</tr>
									<td colspan="8">
...
if ($ind_incapacidad == 1) {
    ?>
			initCKEditorDespacho("txt_observaciones_adicionales_0");
		<?php
}
?>
</script>
