...
function guardar_incapacidad(){
	...
	if($('#hdd_rta_hc').val() == 1){
			...
			var tipo_atencion = $('#cmb_tipo_atencion_0').val();
			var origen_incapacidad = $('#cmb_origen_incapacidad_0').val();
			var prorroga = $('#cmb_prorroga_0').val();
			var fecha_inicial = $('#fechaInicial').val();
			var fecha_final = $('#fechaFinal').val();
			var observaciones = str_encode(eval("CKEDITOR.instances.txt_observaciones_adicionales_0.getData()"));
...
function imprimir_formula_gafas(hdd_id_hc_consulta, esfera_od, cilindro_od, eje_od, adicion_od, esfera_oi, cilindro_oi, eje_oi, adicion_oi, observacion, id_admision, tipo_lente) {
	var hdd_nombre_paciente = $('#hdd_nombre_paciente').val();
