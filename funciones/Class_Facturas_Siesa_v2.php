<?php
	...
	require_once "../db/SQLserver/DbFacturasSQL.php";
	require_once "../funciones/Class_Configuracion_SIESA_SQL.php";
		
	class Class_Facturas_Siesa_v2 {
		
		public function crearFactura($compania, $co_ventas, $bodega_ventas, $nit_tercero, $num_doc_usuario, $id_pago,
				$id_convenio, $id_plan, $ind_tipo_pago, $id_paciente, $lista_productos, $lista_medios_pago, $lista_anticipos,$id_usuario) {
            ...
            $dbFacturasSQL = new DbFacturasSQL();
			...
			$clienteSQLObj = $dbClientesSQL->obtenerClienteDetalle($nit_tercero, $compania);
			$condicion_pago = $clienteSQLObj['Condicion_Pago']; //$classConsultasSiesa->consultarCondicionPagoTercero($compania, $nit_tercero);
			...
			/*$id_pago, $nombre_conector, $compania, $co_ventas, $documento_factura, 
			...
			$resultadoInsertarFactura = $dbFacturasSQL->insertarFacturaPDO($id_pago, "F_DIRECTAS", $compania, $co_ventas, "FEC",
			"1",$fecha_actual, $nit_tercero, "001", "00001", $num_doc_usuario, $id_convenio, $id_plan, $condicion_pago, 
			$id_pago_siesa,"", "");
			...
			foreach ($lista_productos as $prod_aux) {
				if (($ind_tipo_pago == "1" && $prod_aux["valor_cuota"] > 0) || ($ind_tipo_pago == "2" && $prod_aux["valor"] > 0)) {
					...
					$arr_aux = array(
						...
						"VLR_BRUTO" => $valor_unitario * $prod_aux["cantidad"],
			...
			if (count($lista_anticipos) > 0) {
				...
				foreach ($lista_anticipos as $anticipo_aux) {
					$arr_aux = array(
						...
						"CONSECUTIVO_CRUCE"=>$anticipo_aux["numero_anticipo"],
						"VLR_CRUCE" => intval($anticipo_aux["valor"], 10),
						"FECHA_VCTO" => $fecha_actual,
						"FECHA_DSCTO_PP" => $fecha_actual
					);
					
					$arr_cuotas_cxc[$i] = $arr_aux;
					$i++;
				}
				...
				$arr_aux = array(
						...
						"TIPO_DOCTO_CRUCE"=>"",
						"CONSECUTIVO_CRUCE"=> 0,
						"VLR_CRUCE" => 0,//$valor_unitario * $prod_aux["cantidad"],
						"FECHA_VCTO" => $fecha_actual,
						"FECHA_DSCTO_PP" => $fecha_actual
					);
			...
			if ($valor_descuento > 0) {
				...
				//Se crea el array de Descuentos
			...
			if ($resultado_1 == 1) {
				...
				$num_factura = $classConsultasSiesa->consultarFacturaPago($compania, $id_pago);
			} else {
				$num_factura = "";
				$dbComunicacionSiesa->insertar_comunicacion_siesa(22, $id_paciente, "", $id_pago, "", $resultado_1, $id_usuario);
			}
		...
		public function crearEntidadFactura($compania, $cod_prestador, $co_ventas, $num_factura, $bodega_ventas, $arr_productos_aux, $num_doc_tercero, $num_doc_usuario, 
			...
			switch ($tipo_entidad) {//Tipo de la entidad es si va a nivel del documento o del movimiento.
				...
				case 1 : 
						...
						$resultado_aux = $classConectorSiesa->enviarXML(23, $arr_datos, $compania, 0);
						...
						if ($resultado <> 1) {
							...
							$dbComunicacionSiesa->insertar_comunicacion_siesa(23, $id_paciente, "", $id_pago, "", $resultado, $id_usuario);
						}
						return $resultado;
					...
					case 2:
						...
						if(is_array($rowid_movimiento)){
							...
							if($resultado <> 1){
								...
								$dbComunicacionSiesa->insertar_comunicacion_siesa(23, $idPaciente, "", $idPago, "", $resultado, $id_usuario);
							}
							return $resultado;
