...
class Class_Terceros_Siesa {
    ...
    public function crearTerceroCliente($idPaciente, $compania, $id_lugar, $id_admision = "", $id_usuario) {
		...
		//Se consultan los datos de la admisión
		if ($id_admision != "") {
			$admision_obj = $dbAdmision->get_admision($id_admision);
			$zonificacion = $admision_obj["zonificacion"];
		} else {
			$admision_obj = array();
			$zonificacion = "";
		}
		...
		if ($banderaCrearTercero) {//Crea el tercero vigara
			...
			//echo("<br>1<br>");
			//var_dump($rtaTerceroVigaraCreado);
        ...
        if ($banderaCrearTerceroCliente) {
            ...
            if (strlen($rtaTerceroCliente["ImportarDatosXMLResult"]) == 19 && $rtaTerceroCliente["ImportarDatosXMLResult"] == "Importacion exitosa") {
				...
				if ($bol_continuar) {
					if ($zonificacion != "") {
						//Crea la entidad dinámica para zonificación
						$arrayHceoFe = array(
							"Inicial" => array(
								"COMPANIA" => $compania
							),
							...
							"Clientes" => array(
								1 => array(//Asignación del correo electrónico del tercero cliente para la factura electrónica
									"COMPANIA" => $compania,
									"TERCERO_NIT" => $numero_documento,
									"ID_ENTIDAD" => "LOCALIDAD",
									"ID_ATRIBUTO" => 10,
									"DATO_TEXTO" => $zonificacion,
									"ID_MAESTRO_DETALLE" => $ciudadSiesa
								)
							),
						);
    ...
    public function crearTerceroClienteNoPaciente($idTercero, $idPaciente, $compania, $id_admision = "" , $id_usuario ) {
			...
			if (strlen($rtaTerceroCliente["ImportarDatosXMLResult"]) == 19 && $rtaTerceroCliente["ImportarDatosXMLResult"] == "Importacion exitosa") {//19 es la longitud de caracteres para la respuesta: Importacion exitosa
				...
				if ($bol_continuar) {
					if ($zonificacion != ""){
						...
						if(strlen($rtaHceoFe["ImportarDatosXMLResult"]) == 19 && $rtaHceoFe["ImportarDatosXMLResult"] == "Importacion exitosa"){
							$bol_continuar = true;
							$resultado = 1; //Importación exitosa
						} else {
