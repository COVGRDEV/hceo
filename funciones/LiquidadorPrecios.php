	...
	class LiquidadorPrecios {
			...
			if ($id_liq_qx != "") {
				...
				foreach ($lista_procedimientos as $proc_aux) {
					foreach ($lista_valores_liq as $valor_liq_aux) {
						//echo($valor_liq_aux["cod_procedimiento"] == $proc_aux["cod_procedimiento"] && $valor_liq_aux["tipo_bilateral"] == $proc_aux["tipo_bilateral"]);
					...
					foreach ($mapa_vias_aux as $id_via_aux => $lista_procs_aux) {
						...
						foreach ($lista_procs_aux as $proc_aux) {
							...
							foreach ($mapa_liq_qx_det as $tipo_componente => $lista_porcentajes_aux) {
								...
								//echo("Componente: ".$tipo_componente." - Porcentaje: ".$porc_comp_aux." - Porc. bilateral: ".$porc_bila_aux." - Valor: ".$valor_comp_aux."<br />");
								...
								$arr_proc_aux["valor_total"] /*+= $valor_comp_aux;*/ =  $valor_comp_aux;
					...
					/*switch ($tipo_componente) {
						case "E": //Especialista
							$valor_calc = $procedimiento_obj["valor_iss_proc"] * $this->uvr_esp * ($porcentaje / 100) * ($porc_bilateral / 100 + 1) * $frac_iss;
								//echo("VALOR ISS ".$procedimiento_obj["valor_iss_proc"]." UVR ".$this->uvr_esp." PORCENTAJE ".$porcentaje." FRAC ISS ".$frac_iss);
						...
						case "S": //Derechos de sala
							...
							if (isset($rango_sel["valor_iss_2001_d_s"])) {
								$valor_calc = $rango_sel["valor_iss_2001_d_s"] * ($porcentaje / 100) * ($porc_bilateral / 100 + 1) * $frac_iss;
								if ($rango_sel["tipo_valor_iss_2001_d_s"] == "1") {
									$valor_calc *= $procedimiento_obj["valor_iss_proc"];
								}
							}
							
							break;
							
						case "M": //Materiales en insumos
							//Se busca el rango de valor
							
							$rango_sel = array();
							foreach ($this->lista_uvr_m as $rango_aux) {
								if ($procedimiento_obj["valor_iss_proc"] <= $rango_aux["uvr2_iss_2001_m"]) {
									$rango_sel = $rango_aux;
							...
							if (isset($rango_sel["valor_iss_2001_m"])) {
								$valor_calc = $rango_sel["valor_iss_2001_m"] * ($porcentaje / 100) * ($porc_bilateral / 100 + 1) * $frac_iss;
								if ($rango_sel["tipo_valor_iss_2001_m"] == "1") {
									$valor_calc *= $procedimiento_obj["valor_iss_proc"];
								}
							}
						...
						//En el caso en el que el procedimiento tiene el tipo componente T, entonces hace este cálculo.
		...
		public function liquidar_cop_cm($id_paciente, $lista_productos, $tipo_coti_paciente = "", $rango_paciente = "") {
			...
			if(intval($tipo_coti_paciente)>3){
				$tipo_coti_obj = $dbListas->getDetalle($tipo_coti_paciente);
				$tipo_coti_paciente = $tipo_coti_obj["codigo_detalle"];	
			}
			...
			foreach ($mapa_productos as $id_plan => $lista_productos_aux) {
				...
				if ($plan_obj["ind_calc_cc"] == "1") {
					...
					//Cuota moderadora
					if (isset($cuota_moderadora_base_obj["tipo_cotizante"]) && $exento_pamo_paciente == "1") {
