<?php
	class FuncionesHistoriaClinica {
		...
		public function encabezado_historia_clinica($id_paciente, $id_admision, $id_hc = "", $ind_editar = 0, $ind_actualizado = false) {
			...
			if ($id_admision != "") {
				...
				$paciente_obj = $dbPacientes->getPacienteDetalle($id_paciente);
				/** mostrar la foto del paciente en la historia clínica. */
				$url_foto = $dbVariables->getURLFotoPaciente();
				$url_foto = $url_foto["valor_variable"];
				$base64="";
				if($url_foto<>""){
					$path = $url_foto.$paciente_obj["foto_paciente"];
			...
			//Se calcula la edad del paciente con respecto a la fecha de la historia clinica
			...
			$fecha_hc_aux = $funciones_personas->obtenerFecha($hc_obj_aux["fecha_hc_t"]);
    ...
    <div class="contenedor_principal" id="encabezado_hc_principal" style="border: 1px solid #999 !important;">
        ...
        <?php $style = $paciente_obj["foto_paciente"]<>"" ? "style='width:85%; float:left; margin:0px;'" : "style='width:100%; float:left; margin:0px;'"; ?>
		<div id="encabezado_hc" style="display:none;">
			<div style="display:flex; width:100%;  padding:0px;" > 
                ...
                <?php 				
				if(!is_null($paciente_obj["foto_paciente"]) || $paciente_obj["foto_paciente"]<>""){ ?>
	...
	<?php
		...
		public function tieneConsultaOptometria($id_admision) {
			...
			if (!is_null($consulta_obj) && count($consulta_obj) > 1) {
				$resultado = true;
			} else {
