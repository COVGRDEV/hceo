...
<html xmlns="http://www.w3.org/1999/xhtml">
    <head>
        ...
        <script type='text/javascript' src='atencion_postqx_v1.2.js'></script>
    ...
    <body>
        ...
        <div class="contenedor_principal">
            <div class="formulario" id="principal_historia_clinica" style="width: 100%; display: block; ">
                ...
                <table id="lista_pacientes_postqx" class="paginated modal_table" style="width: 100%;  margin: auto;" border='0'>
	        	<thead>
	        	<tr>
					<th colspan="2" style="width: 5%;">Pacientes</th>
					<th style="width: 1%;">Fecha Cx</th>
				...
				foreach($tabla_pacientes as $fila_pacientes){
					...
					$tb_fecha_cx = $dbPostQx->getRestasFechas($fecha_seguimiento_ini, 1);
					$fecha_cx = $tb_fecha_cx['fecha_nueva'];
					...
					<tr class='celdagrid' onclick="ver_registros_hc(<?php echo($id_paciente); ?>, '<?php echo($nombres_apellidos); ?>', '<?php echo($numero_documento); ?>', '<?php echo($tipo_documento); ?>', '<?php echo($telefonos); ?>', '<?php echo($fecha_nacimiento); ?>', '<?php echo($edad_paciente); ?>');">
						...
						<td align="left">OD</td>
						<td align="left"><?php echo($fecha_cx); ?></td>
					<?php
					
					$fechaInicio=strtotime($fecha_seguimiento_ini);
					$fechaFin= $fechaInicio + (86400 * 9);
				...
				<script id='ajax'>
					//<![CDATA[ 
					$(function() {
					    $('#lista_pacientes_postqx', 'table').each(function(i) {
					        $(this).text(i + 1);
					    });
					
					    $('#lista_pacientes_postqx').each(function() {
					        var currentPage = 0;
					        var numPerPage = 10;
					        var $table = $(this);
        ...
        <div id="fondo_negro_postqx" class="d_fondo_negro div_centro_sobre"></div>
  	 	 <div id="d_centro_postqx" class="div_centro div_centro_sobre" style="display:none;">
			<a name="a_cierre_panel" id="a_cierre_panel" href="#" onclick="cerrar_div_centro_postqx();"></a>
			<div style="height: 650px !important; overflow: auto;" id="d_interno_postqx" class="div_interno" > </div>
		 </div>
