...
$(document).ready( function() {  
	...
	// Poner colorpick verde por defecto para campos de AVSC
	// Si están blancos, poner verde=3; ojo: si intensionalmente guardan blando, debería conservarse ese	
	if ($("#hdd_cp_color_3").val()==0) { $("#hdd_cp_color_3").val(2); $("#d_cp_3").trigger("click"); }
	if ($("#hdd_cp_color_5").val()==0) { $("#hdd_cp_color_5").val(2); $("#d_cp_5").trigger("click"); }	
	if ($("#hdd_cp_color_6").val()==0) { $("#hdd_cp_color_6").val(2); $("#d_cp_6").trigger("click"); }
	if ($("#hdd_cp_color_8").val()==0) { $("#hdd_cp_color_8").val(2); $("#d_cp_8").trigger("click"); }
	if ($("#hdd_cp_color_81").val()==0) { $("#hdd_cp_color_81").val(2); $("#d_cp_81").trigger("click"); }
	if ($("#hdd_cp_color_82").val()==0) { $("#hdd_cp_color_82").val(2); $("#d_cp_82").trigger("click"); }
});
...
function validar_optometria() {
	...
	//$("#avsc_media_od").removeClass("borde_error");
	$("#avsc_cerca_od").removeClass("borde_error");
	$("#avsc_lejos_oi").removeClass("borde_error");
	//$("#avsc_media_oi").removeClass("borde_error");
	...
	//$("#subjetivo_media_od").removeClass("borde_error");
	...
	//$("#subjetivo_media_oi").removeClass("borde_error");
	...
	/*if ($("#avsc_media_od").val() == "" && $("#avsc_media_od").attr("disabled") != "disabled") {
		$("#avsc_media_od").addClass("borde_error");
		result = 1;
	}*/
	...
	/*if ($("#avsc_media_oi").val() == "" && $("#avsc_media_oi").attr("disabled") != "disabled") {
		$("#avsc_media_oi").addClass("borde_error");
		result = 1;
	}*/	
	...
	/*if ($("#subjetivo_media_od").val() == "" && $("#subjetivo_media_od").attr("disabled") != "disabled") {
		$("#subjetivo_media_od").addClass("borde_error");
		result = 1;
	}*/	
	...
	/*if ($("#subjetivo_media_oi").val() == "" && $("#subjetivo_media_oi").attr("disabled") != "disabled") {
		$("#subjetivo_media_oi").addClass("borde_error");
		result = 1;
	}*/	
...
function editar_consulta_optometria(tipo, ind_imprimir) {
	...
	if($("#rx_gafas").is(":checked") && $("#cmb_usa_correccion").val()=="1") {var rx_gafas = 1;} else {var rx_gafas = 0;}  
	if($("#rx_ldc").is(":checked") && $("#cmb_usa_correccion").val()=="1") {var rx_ldc = 1;} else {var rx_ldc = 0;}
	if($("#rx_abv").is(":checked") && $("#cmb_usa_correccion").val()=="1") {var rx_abv = 1;} else {var rx_abv = 0;}
	if($("#rx_cxr").is(":checked") && $("#cmb_usa_correccion").val()=="1") {var rx_cxr = 1;} else {var rx_cxr = 0;}
	...
	for (i = 1; i <= canti; i++) {
		...
		if($("#chk_presentes_gafas_"+i).is(":checked")) {var ind_presentes_gafas = 0;} else {var ind_presentes_gafas = 1;}  
	...
	for (i = 1; i <= canti; i++) {
		...
		if($("#chk_presentes_ldc_"+i).is(":checked")) {var ind_presentes_ldc = 0;} else {var ind_presentes_ldc = 1;}  
	...
	for (i = 1; i <= canti; i++) {
		...
		if($("#chk_presentes_abv_"+i).is(":checked")) {var ind_presentes_abv = 0;} else {var ind_presentes_abv = 1;}  
	...
	for (i = 1; i <= canti; i++) {
		...
		if($("#chk_ajuste_"+i).is(":checked")) {var ind_ajuste = 1;} else {var ind_ajuste = 0;}  
	...
	switch (opcion) {
		...
		case 2: // objeto disparador: usa corrección óptica?
			...
			if ($("#cmb_usa_correccion").val()=="1" && $("#cmb_examinado_antes").val()=="1") {
				...
				if ( $("#rx_gafas").is(":checked") ) { $("#seccion_lensometria").show(500); }
			} else {
...
function config_seccion_correccion_optica (sufijo_obj, guardado=0) { 
	...
	if (guardado==1) { //se guardó el formulario 
		if ( $("#cmb_usa_correccion").val()!="1" || $("#rx_"+sufijo_obj).is(":checked")==false ) { 
			...
			if (sufijo_obj=="gafas") { $("#seccion_lensometria").empty(); }	
	...
	if ($("#rx_"+sufijo_obj).is(":checked")) { 
		...
		if (sufijo_obj=="gafas") { $("#seccion_lensometria").show("slow"); } 
	} else { 
		$("#seccion_"+sufijo_obj).hide("slow"); 
		if (sufijo_obj=="gafas") { $("#seccion_lensometria").hide("slow"); } 
	} 
}
...
function agregar_frm(sufijo_obj) { 
	...
	switch (sufijo_obj) {
		case "gafas": cod_objeto=4; div_destino="div_ajax_gafas"; break; 
		case "ldc": cod_objeto=5; div_destino="div_ajax_ldc"; break; 
		case "abv": cod_objeto=6; div_destino="div_ajax_abv"; break; 
		case "cxr": cod_objeto=7; div_destino="div_ajax_cxr"; break; 
	}
...
function fin_agregar_frm (fila, cod_objeto) {
	...
	var vector;
	//div_origen="div_tmp_ajax";
	
	switch (cod_objeto) {
		case 4: div_origen="div_ajax_gafas"; div_destino="div_gafas"; break; 
		case 5: div_origen="div_ajax_ldc"; div_destino="div_ldc"; break;
		case 6: div_origen="div_ajax_abv"; div_destino="div_abv"; break;
		case 7: div_origen="div_ajax_cxr"; div_destino="div_cxr"; break;
	}
...
function cmb_dependientes(tipo_combo, fila) { 
	...
	switch (tipo_combo) {
		...
		case 2: //cmb_tipo_ldc_[i]
			...
			//if (valor_padre==360) { //blandos
			/*if (1==1) {
				//$("#d_ldc_blandos_"+fila).show(); 
				//$("#cmb_tipo_ldc_det_"+fila).attr('disabled', true); 
				$("#cmb_disenioldc_"+fila).attr('disabled', false); 
				$("#cmb_tipoduracion_ldc_"+fila).attr('disabled', false); 
				$("#cmb_tipoduracion_det_ldc_"+fila).attr('disabled', false); 
			} else {
				//$("#d_ldc_blandos_"+fila).hide(); 
				//$("#cmb_tipo_ldc_det_"+fila).attr('disabled', false); 
				$("#cmb_disenioldc_"+fila).attr('disabled', true); 
				$("#cmb_disenioldc_"+fila).val(""); 
				$("#cmb_tipoduracion_ldc_"+fila).attr('disabled', true); 
				$("#cmb_tipoduracion_ldc_"+fila).val(""); 
				$("#cmb_tipoduracion_det_ldc_"+fila).attr('disabled', true); 
				$("#cmb_tipoduracion_det_ldc_"+fila).val(""); 
			}
			*/
			
			//if (valor_padre==382 || valor_detalle==) { //otro				
...
function config_dr (tipo_obj, fila) { 
	//obj=$("#"+obj_disparador.id); 
	//ojo=obj.attr("id").substr(-2, 2); 	
	switch (tipo_obj) {
