<?php
	...
	switch ($opcion) {
		case "1": //Se imprime la fórmula de gafas
			...
			//var_dump("Entró acá ".$hdd_id_hc_consulta);
			...
			$pdf->Cell(0, 4, ajustarCaracteres("CENTRO OFTALMOLÓGICO VIRGILIO GALVIS"), "C", "C", "C");
			$pdf->SetY(18);
			$pdf->SetX(90);
			$pdf->SetFont("Arial", "", 7);
        	...
        	$pdf->Cell(18, 4, ajustarCaracteres("Sede:"), 0, 0, "L");
        	$pdf->SetFont("Arial", "", 9);
        	$pdf->Cell(60, 4, ajustarCaracteres($consulta_optometria_obj["lugar_cita"]), 0, 0, "L");
        	$pdf->Cell(55, 4, "", 0, 0, "L");
        	$pdf->SetFont("Arial", "B", 9);
        	$pdf->Cell(12, 4, ajustarCaracteres("Fecha:"), 0, 0, "L");
        	$pdf->SetFont("Arial", "", 9);
        	$pdf->Cell(20, 4, ajustarCaracteres($hdd_fecha_admision), 0, 1, "L");
        	$pdf->Ln(1);
        
			$pdf->SetFont("Arial", "B", 9);
        	$pdf->Cell(18, 4, ajustarCaracteres("Nombre:"), 0, 0, "L");
        	$pdf->SetFont("Arial", "", 9);
        	$pdf->Cell(60, 4, ajustarCaracteres($utilidades->convertir_a_mayusculas($hdd_nombre_paciente)), 0, 0, "L");
			$pdf->Cell(55, 4, "", 0, 0, "L");
			$pdf->SetFont("Arial", "B", 9);
        	$pdf->Cell(12, 4, ajustarCaracteres("No HC: "), 0, 0, "L");
			$pdf->SetFont("Arial", "", 9);
			$pdf->Cell(20, 4, ajustarCaracteres($numero_documento), 0, 0, "L");
			$pdf->Ln(5);
			...
			/*
			$pdf->SetFont("Arial", "B", 9);
			$pdf->Cell(12, 4, ajustarCaracteres("Sede:"), 0, 0, "L");
			$pdf->SetFont("Arial", "", 9);
			$pdf->Cell(20, 4, ajustarCaracteres($consulta_optometria_obj["lugar_cita"]), 0, 0,"L");
			$pdf->Ln(5); 	
			$pdf->SetFont("Arial", "B", 9);
			$pdf->Cell(12, 4, ajustarCaracteres("No HC: "), 0, 0, "L");
			$pdf->SetFont("Arial", "", 9);
			$pdf->Cell(20, 4, ajustarCaracteres($numero_documento), 0, 1, "L");
			$pdf->Ln(1);
			$pdf->SetFont("Arial", "B", 9);
			$pdf->Cell(12, 4, ajustarCaracteres("Fecha:"), 0, 0, "L");
			$pdf->SetFont("Arial", "", 9);
			...
			$pdf->Cell(18, 4, ajustarCaracteres("Nombre:"), 0, 0, "L");
			$pdf->SetFont("Arial", "", 9);
			$pdf->Cell(100, 4, ajustarCaracteres($utilidades->convertir_a_mayusculas($hdd_nombre_paciente)), 0, 0, "L");
			if ($tipo_documento != "" && $numero_documento != "") {
			$pdf->SetFont("Arial", "B", 9);
			$pdf->Cell(40, 4, ajustarCaracteres($tipo_documento.":"), 0, 0, "R");
			$pdf->SetFont("Arial", "", 9);
			$pdf->Cell(20, 4, ajustarCaracteres($numero_documento), 0, 0, "L");
			$pdf->Ln(5);
			$pdf->SetFont("Arial", "B", 9);
			$pdf->Cell(16, 4, ajustarCaracteres("Tipo usuario: "), 0, 0, 'L');
			$pdf->SetFont("Arial", "", 9);
			$pdf->Cell(40, 4, ajustarCaracteres($tipo_usuario_t), 0, 0, 'R');
			}
			/*$pdf->Ln();
			$pdf->SetFont("Arial", "B", 9);
			$pdf->Cell(18, 4, ajustarCaracteres("Tipo de cotizante y plan :"), 0, 0, "L");
			$pdf->SetFont("Arial", "", 9);
			$pdf->Cell(100, 4, ajustarCaracteres($consulta_optometria_obj[""] != "" ? $tipo_lente : "-"), 0, 0, "L");*/
			$pdf->Ln(5);
       		$x_aux = $pdf->GetX();
       		$y_aux = $pdf->GetY();
			...
			$pdf->Cell(28, 6, ajustarCaracteres($consulta_optometria_obj["agudezaL_ojoI"] !="" ? $consulta_optometria_obj["agudezaL_ojoI"] : "-"), 1, 1, "C", true);
			...
			//imprimir_firma($usuario_prof_opt_obj["id_profesional"], $pdf, $nombre_usuario_alt);
			
			$pdf->Ln(9);
			$pdf->SetFont("Arial", "B", 9);
			$pdf->Cell(195, 4, ajustarCaracteres("¡Muy Importante!"), 0, 1, "L", true);
			$pdf->SetFont("Arial", "", 7);
			$pdf->Cell(195, 3, ajustarCaracteres("Hemos formulado la mejor y m&aacute;s adecuada tecnolog&iacute;a en lentes para usted. A la &oacute;ptica"), 0, 1, "L", true);
			$pdf->Cell(195, 3, ajustarCaracteres("que vaya, demande excelente atenci&oacute;n y exija precisi&oacute;n y garant&iacute;a. No permita cambio"), 0, 1, "L", true);
