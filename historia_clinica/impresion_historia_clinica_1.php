<?php
    ...
    class PDF extends FPDI {
		...
		function setData($nombre_tipo_reg, $fecha_hc, $nombre_completo, $tipo_documento, $numero_documento, $nombre_convenio, $nombre_plan, $fecha_nacimiento_t, $edad_texto, $sexo_t, $presion_arterial, $pulso, $nombre_acompa, $edad_funcion,$foto, $url, $tipo_usuario_t) {
		...
		function Header() {
			...
			$this->SetTitle(utf8_decode($this->nombre_completo));
			// Logo
			$this->Image("../".Configuracion::$LOGO_INICIO, 10, 10, 55);
			if($this->foto <> "NULL" && $this->foto <> ""){
				$this->Cell(172, 4);
				$this->Cell(25,27, $this->Image($this->url.$this->foto,182, 10,25,27),1,0,'R');
				$x = 60;
			}else{
				$x = 65;
			}
			...
			//$this->Cell(0, 4, ajustarCaracteres("Tipo de usuario: ".$this->tipo_usuario), 0,0,"L");
			$this->Ln();
			$this->Cell($x, 4);
			$this->Cell($x, 4, ajustarCaracteres($this->tipo_documento.': '.$this->numero_documento), 0, 0, 'L');
			$this->Cell($x, 4, ajustarCaracteres('Fecha: '.$funciones_persona->obtenerFecha6($this->fecha_hc)), 0, 0, 'L');
			...
			if ($this->nombre_convenio != "" || $texto_aux != "") {
				...
				if ($this->nombre_convenio != "") {
					$this->Cell($x, 4, ajustarCaracteres($this->nombre_convenio.' / '.$this->nombre_plan), 0, 0, 'L');
				} else {
					$this->Cell($x, 4);
				}
				$this->Cell($x, 4, ajustarCaracteres($texto_aux), 0, 0, 'L');
				$this->Ln();
			}
			...
			$this->Cell($x, 4);
			$this->Cell($x, 4, ajustarCaracteres('Fecha de nacimiento: '.$funciones_persona->obtenerFecha6($this->fecha_nacimiento_t)), 0, 0, 'L');
			//$this->Cell(33, 4, ajustarCaracteres('Edad: '.$this->edad_texto), 0, 0, 'L');
			$this->Cell(130, 4, ajustarCaracteres('Edad y tipo: '.$this->edad_funcion.' / '.$this->tipo_usuario_t), 0, 0, 'L');
	...
	function imprimir_antecedentes($id_hc, $texto_antecedentes, $ind_antec_cx_refrac, $pdf, $pdfHTML, $ind_ojos) {
		require_once("../db/DbAntecedentes.php");
		$db_antecedentes = new DbAntecedentes();
		...
		$pdf->SetFont("Arial", "", 9);
		if ($ind_ojos) {
			if ($ind_antec_cx_refrac == "1") {
				$pdf->Cell(125, 5, ajustarCaracteres("Con antecedentes de cirugía refractiva (LASIK, PRK, T-PRK, Queratotomía radial, etc.)."), 0, 1, 'L');
				$pdf->Ln(2);
			} else if ($ind_antec_cx_refrac == "0") {
				$pdf->Cell(125, 5, ajustarCaracteres("Sin antecedentes de cirugía refractiva (LASIK, PRK, T-PRK, Queratotomía radial, etc.)."), 0, 1, 'L');
				$pdf->Ln(2);
			}
		}
		
		$texto_aux = ajustarCaracteres(trim($texto_antecedentes) != "" ? $texto_antecedentes : "<p>(Sin antecedentes)</p>");
		$pdfHTML->WriteHTML($texto_aux, $pdf);
		$pdf->Ln(3);
		
		$lista_antecedentes_det = $db_antecedentes->get_lista_hc_antecedentes_medicos_det($id_hc);
		if (count($lista_antecedentes_det) > 0) {
			$id_antecedente_ant = "";
			foreach ($lista_antecedentes_det as $antecedente_det_aux) {
				if ($antecedente_det_aux["id_antecedentes_medicos"] != $id_antecedente_ant) {
					if ($id_antecedente_ant != "") {
						//Se traza la línea de cierre anterior
						$x_aux = $pdf->GetX();
						$y_aux = $pdf->GetY();
					...
					$pdf->Cell(195, 5, ajustarCaracteres($antecedente_det_aux["nombre_antecedentes_medicos"]), 0, 1, 'L');
					
					$pdf->Cell(115, 5, ajustarCaracteres("Descripción"), "TL", 0, 'L');
					$pdf->Cell(55, 5, ajustarCaracteres("Profesional"), "T", 0, 'L');
					...
					$pdf->SetFont("Arial", "", 9);
					$pdf->Cell(115, 5, ajustarCaracteres($antecedente_det_aux["nombre_tipo"]), "L", 0, 'L');
					$pdf->Cell(55, 5, ajustarCaracteres($antecedente_det_aux["nombre_profesional"]), 0, 0, 'L');
					$pdf->Cell(25, 5, ajustarCaracteres($antecedente_det_aux["fecha_det_t"]), "R", 1, 'C');
				
				$id_antecedente_ant = $antecedente_det_aux["id_antecedentes_medicos"];
			}
			
			if (count($lista_antecedentes_det) > 0) {
				//Se traza la línea de cierre del último grupo
				$x_aux = $pdf->GetX();
				$y_aux = $pdf->GetY();
				$pdf->Line($x_aux, $y_aux, $x_aux + 195, $y_aux);
			}
	...
	function imprimir_tonometrias($id_hc, $id_paciente, $id_admision, $pdf) {
		...
		//var_dump($consulta_optometria_obj);
				...
				switch ($id_tipo_reg_base) {
					case "1": //CONSULTA DE OPTOMETRIA
						...
						if (count($consulta_optometria_obj) > 0) {
							...
							$pdf->SetFont("Arial", "", 9);
							$pdf->Cell(45, 5, ajustarCaracteres($consulta_optometria_obj["lugar_cita"]), 0, 0, 0);
							$pdf->Ln(5);
							$pdf->SetFont("Arial", "B", 9);
							$pdf->Cell(12, 5, ajustarCaracteres("Teléfono:"), 0, 0, "L");
							$pdf->SetFont("Arial", "", 9);
							$pdf->Cell(20, 5, ajustarCaracteres($consulta_optometria_obj["tel_sede_det"]), 0, 0,"R");
							$pdf->Ln(5);
							$pdf->SetFont("Arial", "B", 9);
							$pdf->Cell(12, 5, ajustarCaracteres("Dirección:"), 0, 0, "L");
							$pdf->SetFont("Arial", "", 9);
							$pdf->Cell(144, 5, ajustarCaracteres($consulta_optometria_obj["dir_sede_det"]), 0, 1,"R");
							...
							$pdf->Ln(2);
							$pdf->SetFont("Arial", "B", 9);
							$pdf->MultiCell(195, 4, ajustarCaracteres("M. de C.: ".(trim($admision_obj["motivo_consulta"]) != "" ? $admision_obj["motivo_consulta"] : "-")), 0, 'L');
							$pdf->Ln(2);
							...
							$pdf->SetFont("Arial", "", 9);
							$pdf->Cell(20, 5, ajustarCaracteres($consulta_optometria_obj["dominancia_ocular"]), 0, 1, 'L');
							...
							//Cicloplejia
							if ($consulta_optometria_obj["cicloplejio_esfera_od"] != "" || $consulta_optometria_obj["cicloplejio_cilindro_od"] != "" ||
									$consulta_optometria_obj["cicloplejio_eje_od"] != "" || $consulta_optometria_obj["cicloplejio_esfera_oi"] != "" ||
									$consulta_optometria_obj["cicloplejio_cilindro_oi"] != "" || $consulta_optometria_obj["cicloplejio_eje_oi"] != "" ||
									$consulta_optometria_obj["cicloplejio_lejos_od"] != "0" || $consulta_optometria_obj["cicloplejio_lejos_oi"] != "0") {
								$pdf->SetFont("Arial", "", 9);
					...
					case "2": //CONSULTA DE OFTALMOLOGIA
						...
						if (count($consulta_oftalmologia_obj) > 0) {
							...
							imprimir_antecedentes($id_hc, $consulta_oftalmologia_obj["desc_antecedentes_medicos"], $consulta_oftalmologia_obj["ind_antec_cx_refrac"], $pdf, $pdfHTML, true);
					...
					case "19": //CONSULTA DE CONTROL DE OPTOMETRIA
						...
						if (count($consulta_control_optometria_obj) > 0) {
							...
							// Datos sobre lentes y vigencias del mimso
					...
					case "56": //CONSULTA DERMATOLÓGICA
						...
						if (count($consulta_dermatologia_obj) > 0) {
							...
							imprimir_antecedentes($id_hc, $consulta_dermatologia_obj["desc_antecedentes_medicos"], $consulta_dermatologia_obj["ind_antec_cx_refrac"], $pdf, $pdfHTML, false);
							$pdf->Ln(2);
					...
					default:
						...
						if (count($consulta_evolucion_obj) > 0) {
							...
							//Se imprimen los antecedentes
							imprimir_antecedentes($id_hc, $consulta_evolucion_obj["desc_antecedentes_medicos"], $consulta_evolucion_obj["ind_antec_cx_refrac"], $pdf, $pdfHTML, true);
							$pdf->Ln(2);
