<?php
	...
	switch ($tipo_reporte) {
		case "1": //Reporte general
			...
			$lista_estados_sn = array();
			$lista_estados_sn[0]["id_estado_prog"]= "432";
			$lista_estados_sn[0]["sel_estado_prog"]= 1;
			$lista_estados_sn[1]["id_estado_prog"]= "433";
			$lista_estados_sn[1]["sel_estado_prog"]= 0;
			$lista_estados_sn[2]["id_estado_prog"]= "434";
			$lista_estados_sn[2]["sel_estado_prog"]= 0;
			$lista_estados_sn[3]["id_estado_prog"]= "435";
			$lista_estados_sn[3]["sel_estado_prog"]= 0;
			
			$lista_programacion_cx_sn_prg = $dbProgramacionCx->getListaProgramacionCxFechas($fecha_ini, $fecha_fin, $tipo_fecha, $tipo_concepto, $cod_concepto, $id_usuario_prof, $lista_estados_sn);
			...
			$objPHPExcel->setActiveSheetIndex(1)->getColumnDimension("C")->setWidth(30);
			...
			$objPHPExcel->setActiveSheetIndex(1)->getColumnDimension("I")->setWidth(30);
			$objPHPExcel->setActiveSheetIndex(1)->getColumnDimension("J")->setWidth(40);
			...
			$objPHPExcel->setActiveSheetIndex(1)->getColumnDimension("Q")->setWidth(30);
			$objPHPExcel->setActiveSheetIndex(1)->getColumnDimension("R")->setWidth(30);
			$objPHPExcel->setActiveSheetIndex(1)->getColumnDimension("S")->setWidth(20);
			$objPHPExcel->setActiveSheetIndex(1)->getColumnDimension("T")->setWidth(20);
			$contador_linea = 1;
			...
			$objPHPExcel->getActiveSheet()
						...
						->setCellValue("F".$contador_linea, "TELÉFONO 1")
						->setCellValue("G".$contador_linea, "TELÉFONO 2")
						->setCellValue("H".$contador_linea, "CORREO")
						->setCellValue("I".$contador_linea, "CONVENIO")
						->setCellValue("J".$contador_linea, "PROFESIONAL")
						->setCellValue("K".$contador_linea, "PRODUCTOS/SERVICIOS")
						->setCellValue("L".$contador_linea, "TIPO LENTE")
						->setCellValue("M".$contador_linea, "SERIAL LENTE")
						->setCellValue("N".$contador_linea, "PODER LENTE")
						->setCellValue("O".$contador_linea, "ESTADO")
						->setCellValue("P".$contador_linea, "FECHA REGISTRO")
						->setCellValue("Q".$contador_linea, "HORA REGISTRO")
						->setCellValue("R".$contador_linea, "HABEAS DATA");
			
			$objPHPExcel->getActiveSheet()->getStyle("A".$contador_linea.":T".$contador_linea)->getFont()->setBold(true);
			
			$contador_linea++;
			
			foreach ($lista_programacion_cx_sn_prg as $prog_cx_aux_sn) {
				$lista_programacion_cx_det = $dbProgramacionCx->getListaProgramacionCxDet($prog_cx_aux_sn["id_prog_cx"]);
				...
				//Nombre del paciente
				$habeas_data = "";
				if($prog_cx_aux_sn["ind_habeas_data"]=="1"){
					$habeas_data="Autoriza el uso de datos";
				}else{
					$habeas_data="No Autoriza";
				}
				$nombre_completo = $funciones_persona->obtenerNombreCompleto($prog_cx_aux_sn["nombre_1"], $prog_cx_aux_sn["nombre_2"], $prog_cx_aux_sn["apellido_1"], $prog_cx_aux_sn["apellido_2"]);
					$objPHPExcel->getActiveSheet()
						->setCellValue("A".$contador_linea, "FECHA PROGRAMADA")
						->setCellValue("B".$contador_linea, "HORA PROGRAMADA")
						->setCellValue("C".$contador_linea, "TIPO DOCUMENTO")
						...
						->setCellValue("R".$contador_linea, "HABEAS DATA");
				$objPHPExcel->getActiveSheet()
					->setCellValue("A".$contador_linea, $prog_cx_aux_sn["fecha_prog_t"])
					->setCellValue("B".$contador_linea, $prog_cx_aux_sn["hora_prog_t"])
					->setCellValue("C".$contador_linea, $prog_cx_aux_sn["tipo_documento"])
					->setCellValue("D".$contador_linea, $prog_cx_aux_sn["numero_documento"])
					->setCellValue("E".$contador_linea, $nombre_completo)
					->setCellValue("F".$contador_linea, $prog_cx_aux_sn["telefono_1"])
					->setCellValue("G".$contador_linea, $prog_cx_aux_sn["telefono_2"])
					->setCellValue("H".$contador_linea, $prog_cx_aux_sn["email"])
					->setCellValue("I".$contador_linea, $prog_cx_aux_sn["nombre_convenio"])
					
					->setCellValue("J".$contador_linea, $prog_cx_aux_sn["nombre_usuario_prof"]." ".$prog_cx_aux_sn["apellido_usuario_prof"])
					->setCellValue("K".$contador_linea, $cadena_productos)
					->setCellValue("L".$contador_linea, $cadena_tipos_lentes)
					->setCellValue("M".$contador_linea, $cadena_seriales_lentes)
					->setCellValue("N".$contador_linea, $cadena_poderes_lentes)
					->setCellValue("O".$contador_linea, $prog_cx_aux_sn["estado_prog"])
					->setCellValue("P".$contador_linea, $prog_cx_aux_sn["fecha_crea_t"])
					->setCellValue("Q".$contador_linea, $prog_cx_aux_sn["hora_crea_t"])
					->setCellValue("R".$contador_linea, $habeas_data);
				
				$contador_linea++;
			}
					
			$objWorkSheet->setTitle("Sin programar");
