	...
	switch ($tipoReporte) {
		case "1": //Reporte general
			...
			if($diferencia_dias >= 34){
				...
				$objPHPExcel->setActiveSheetIndex(0)->getColumnDimension("X")->setWidth(16);
				$objPHPExcel->setActiveSheetIndex(0)->getColumnDimension("Y")->setWidth(16);
				$objPHPExcel->setActiveSheetIndex(0)->getColumnDimension("AJ")->setWidth(30);
				...
				$objPHPExcel->getActiveSheet()
							...
							->setCellValue("G".$contador_linea, "PROFESIONAL FORMULA")
							->setCellValue("H".$contador_linea, "PROFESIONAL QUE ATIENDE")
							->setCellValue("I".$contador_linea, "OPTÓMETRA")
							->setCellValue("J".$contador_linea, "TÉCNICO")
							->setCellValue("K".$contador_linea, "TIPO DE ATENCIÓN")
							->setCellValue("L".$contador_linea, "CÓDIGO CONCEPTO")
							->setCellValue("M".$contador_linea, "NOMBRE CONCEPTO")
							->setCellValue("N".$contador_linea, "BILATERAL")
							->setCellValue("O".$contador_linea, "TIPO DOCUMENTO")
							->setCellValue("P".$contador_linea, "NÚMERO DOCUMENTO")
							->setCellValue("Q".$contador_linea, "NOMBRE PACIENTE")
							->setCellValue("R".$contador_linea, "TIPO DOCUMENTO TERCERO")
							->setCellValue("S".$contador_linea, "NÚMERO DOCUMENTO TERCERO")
							->setCellValue("T".$contador_linea, "NOMBRE TERCERO")
							->setCellValue("U".$contador_linea, "USUARIO REG. PAGO")
							->setCellValue("V".$contador_linea, "CANTIDAD")
							->setCellValue("W".$contador_linea, "VALOR UNITARIO")
							->setCellValue("X".$contador_linea, "VALOR TOTAL")
							->setCellValue("Y".$contador_linea, "COPAGO");
				
				$arr_letras = array("Z", "AA", "AB", "AC", "AD", "AE", "AF", "AG","AH", "AI", "AK", "AL", "AM", "AN", "AO", "AP", "AQ", "AR", "AS", "AT", "AU", "AV", "AW", "AX", "AY", "AZ");
				
				$max_col_letra = "Z";
				$objPHPExcel->getActiveSheet()->getStyle("AJ".$contador_linea)->getFont()->setBold(true);
				$objPHPExcel->getActiveSheet()->setCellValue("AJ".$contador_linea, "INDICADOR HABEAS DATA");
				...
				foreach ($rta_atenciones_aux as $value) {
					...
					$nombre_tecnico = "";
					if ($value["id_admision"] != "") {
						$lista_tecnicos_aux = $dbUsuarios->getListaUsuariosAdmisionPerfil($value["id_admision"], 16);
						if (count($lista_tecnicos_aux) > 0) {
							$nombre_tecnico = $lista_tecnicos_aux[0]["nombre_usuario"]." ".$lista_tecnicos_aux[0]["apellido_usuario"];
						}
					}
					...
					$objPHPExcel->getActiveSheet()
						...
						->setCellValue("G".$contador_linea, $value["profesional_formula"])
						->setCellValue("H".$contador_linea, $value["medico"])
						->setCellValue("I".$contador_linea, $nombre_optometra)
						->setCellValue("J".$contador_linea, $nombre_tecnico)
						->setCellValue("K".$contador_linea, ($value["nombre_tipo_cita"] != "" ? $value["nombre_tipo_cita"] : "(Sin admisión)"))
						->setCellValue("L".$contador_linea, $value["cod_insumo"])
						->setCellValue("M".$contador_linea, $value["nombre_insumo"])
						->setCellValue("N".$contador_linea, $tipo_bilateral_aux)
						->setCellValue("O".$contador_linea, $value["tipo_documento"])
						->setCellValue("P".$contador_linea, $value["numero_documento"])
						->setCellValue("Q".$contador_linea, $funciones_persona->obtenerNombreCompleto($value["apellido_1"], $value["apellido_2"], $value["nombre_1"], $value["nombre_2"]))
						->setCellValue("R".$contador_linea, $value["tipo_documento_tercero"])
						->setCellValue("S".$contador_linea, $numero_documento_tercero)
						->setCellValue("T".$contador_linea, $value["nombre_tercero"])
						->setCellValue("U".$contador_linea, $value["usuario_registra_pago"])
						->setCellValue("V".$contador_linea, $value["cantidad"])
						->setCellValue("W".$contador_linea, $value["valor"])
						->setCellValue("X".$contador_linea, $value["cantidad"] * $value["valor"])
						->setCellValue("Y".$contador_linea, $value["cantidad"] * $value["valor_cuota"])
						->setCellValue("AJ".$contador_linea,$mensaje);
					...
					foreach ($lista_tipos_pago as $i => $tipo_pago_aux) {
						...
						if($medio_pago == "100"){
							if ($ind_pago_contingencia) {
								$objPHPExcel->getActiveSheet()->setCellValue("AH".$contador_linea, "SÍ");
							}
						}
