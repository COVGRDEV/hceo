<?php
	...
	switch ($opcion) {
		...
		case "4":// se genera reporte de coomeva
			...
			$lista_pagos= array();
			$lista_pagos = $dbPagos->get_reporte_coomeva_excel($fecha_inicial, $fecha_final);
			$pdf->bordeMulticell = 1; //Si la tabla tiene borde. 1 = Con borde. 2 = Sin borde
			...
			$pdf->Ln(5);
			$pdf->SetFont("Arial", "B", 10);
			$pdf->Cell(0, 0, ajustarCaracteres("Reporte Coomeva MP"), 0, 0, "C");
			$pdf->Ln(10);
			...
			$pdf->SetAligns2(array("C"));
			$pdf->SetWidths2(array(195));
			$pdf->Row2(array($fecha_inicial." - ".$fecha_inicial));
			$pdf->SetAligns2(array("C", "C", "C", "C", "C", "C", "C", "C", "C"));
			$pdf->SetWidths2(array(35,20,20,20,20,20,20,20,20));
			$pdf->Row2(array("Nombre", "No.Doc", "Cod Ciex", "CUPS", "No.Auto" , "Fecha" , "Tarifa Negociada" , "Cupón" , "Valor" ));
			$pdf->SetFont("Arial", "",6);
			
			for($i = 0; $i < count($lista_pagos); $i++) {
				$pago_aux = $lista_pagos[$i];
				
				$pdf->SetAligns2(array("L", "L", "C", "C", "L", "C", "C", "C", "C"));
				$pdf->SetWidths2(array(35,20,20,20,20,20,20,20,20));
				$pdf->Row2(array($pago_aux["paciente"], $pago_aux["identificacion"], $pago_aux["cod_ciex"], $pago_aux["CUPS"], $pago_aux["num_autorizacion"],$pago_aux["fecha_autorizacion"],$pago_aux["tarifa_negociada"],$pago_aux["cupon_pago_directo"],$pago_aux["valor_a_pagar"] ));
			}
			...
			$pdf->Output();
			//Se guarda el documento pdf
			$nombreArchivo = "../tmp/reporte_coomeva_".$_SESSION["idUsuario"].".pdf";
			$pdf->Output($nombreArchivo, "F");
		?>
		<input type="hidden" name="hdd_archivo_pdf" id="hdd_archivo_pdf" value="<?php echo($nombreArchivo); ?>" />
		<?php
