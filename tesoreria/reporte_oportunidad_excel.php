<?php
	...
	switch ($tipoReporte) {
		...
		case "5": //Reporte para Coomeva
			
			@$fecha_inicial = $utilidades->str_decode($_POST["hdd_fecha_inicial_coomeva"]);
			@$fecha_final = $utilidades->str_decode($_POST["hdd_fecha_final_coomeva"]);
			@$id_convenio = $utilidades->str_decode($_POST["hdd_convenio_coomeva"]);
			
			
			$lista_pagos= array();
			
			$lista_pagos = $dbPagos->get_reporte_coomeva_excel($fecha_inicial, $fecha_final, $id_convenio);
			...
			$objPHPExcel->getActiveSheet()
						->setCellValue("A1", "Entidad")
						->setCellValue("B1", "COOMEVA")
						->setCellValue("A2", "Fechas")
						->setCellValue("B2", $fecha_inicial." - ".$fecha_final);
			
			$objPHPExcel->getActiveSheet()
						->setCellValue("A4", "#")
						->setCellValue("B4", "NÚMERO DE LA AUTORIZACIÓN")
						->setCellValue("C4", "CÓDIGO DEL DIAGNOSTICO")
						->setCellValue("D4", "NOMBRE COMPLETO DEL PACIENTE")
						->setCellValue("E4", "DOCUMENTO DE IDENTIDAD")
						->setCellValue("F4", "CUPS")
						->setCellValue("G4", "FECHA AUTORIZACIÓN")
						->setCellValue("H4", "TARIFA NEGOCIADA")
						->setCellValue("I4", "CUPÓN DE PAGO")
						->setCellValue("J4", "VALOR NETO A PAGAR");
			...
			for ($i = 0; $i < count($lista_pagos); $i++) {
				...
				$auto++;
				$objPHPExcel->getActiveSheet()
							->setCellValue("A".$num_linea, $auto)
							->setCellValue("B".$num_linea, $pago_aux["num_autorizacion"])
							->setCellValue("C".$num_linea, $pago_aux["cod_ciex"])
							->setCellValue("D".$num_linea, $pago_aux["paciente"])
							->setCellValue("E".$num_linea, $pago_aux["identificacion"])
							->setCellValue("F".$num_linea, $pago_aux["CUPS"])
							->setCellValue("G".$num_linea, $pago_aux["fecha_autorizacion"])
							->setCellValue("H".$num_linea, $pago_aux["tarifa_negociada"])
							->setCellValue("I".$num_linea, $pago_aux["cupon_pago_directo"])
							->setCellValue("J".$num_linea, $pago_aux["valor_a_pagar"]);
			...
			//Se renombra la hoja
			$objPHPExcel->getActiveSheet()->setTitle("Coomeva");
			...
			//Se borra el reporte previamente generado por el usuario
			//@unlink("./tmp/reporte_coomeva".$id_usuario.".xlsx");
			
			// Save Excel 2007 file
			$id_usuario = $_SESSION["idUsuario"];
			$objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, "Excel2007");
			//$objWriter->save("./tmp/reporte_colpatria_2_".$id_usuario.".xlsx");
			$objWriter->save("./tmp/reporte_coomeva".$id_usuario.".xlsx");
		?>
        <form name="frm_reporte_coomeva" id="frm_reporte_coomeva" method="post" action="tmp/reporte_coomeva<?php echo($id_usuario); ?>.xlsx">
        </form>
        <script id="ajax" type="text/javascript">
			document.getElementById("frm_reporte_coomeva").submit();
		</script>
		<?php
		break;	
